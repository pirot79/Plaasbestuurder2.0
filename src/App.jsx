...
const TRANSLATIONS = {
  ...
  en: {
    ...
    history:"History", export:"Export", transactions:"Transactions",
...
// Active language — read from localStorage, default Afrikaans
// This is module-level so it's available to constants defined before any React component.
// React components read `useLang()` hook instead and re-render on change.
...
// T is a proxy so every T.key access always reads the current language
...
// ── useLang hook — call inside any component that needs to re-render on lang switch
function useLang() {
...
function CategoryGrid({ label, values, onChange, max }) {
  const [,] = useLang(); // re-render on language switch
  return (
...
// ══════════════════════════════════════════════════════════════════════════════
function PaddockDetailModal({ p, onClose, addInspection }) {
  const [,] = useLang(); // re-render on language switch
      ...
      <div className="modal" onClick={e=>e.stopPropagation()}>
        ...
        {totalCattle(p.cattle||emptyCattle()) > 0 && (
          <div style={{marginBottom:10}}>
            ...
            <div className="stats">
              {CATTLE_CATS.map(c=>(
                <div key={c} className="stat">
                  <div className="stat-val" style={{color:"var(--teal)"}}>{(p.cattle||emptyCattle())[c]}</div>
                  <div className="stat-lbl">{CATTLE_LABEL()[c]}</div>
                </div>
              ))}
              <div className="stat" style={{gridColumn:"span 2"}}>
                <div className="stat-val">{totalCattle(p.cattle||emptyCattle())}</div>
                <div className="stat-lbl">{T.total}</div>
              </div>
            </div>
          </div>
          ...
          {history.map((h,i) => (
            <div key={h.id||i} className="hist-entry">
              <div className="hist-meta">
                ...
                <span style={srcBadge(h.source)}>{srcLabel(h.source)}</span>
              </div>
        ...
        {showInsp && (
          <div className="extra-section" style={{marginTop:8}}>
            <h4>📋 {T.inspection}</h4>
            <div className="form-group">
              <label className="form-label">🐑 {T.sheepScore}</label>
              <ScoreSelector value={iSheep} onChange={setISheep} />
            </div>
            <div className="form-group">
              <label className="form-label">🐐 {T.goatScore}</label>
              <ScoreSelector value={iGoat} onChange={setIGoat} />
            </div>
            <div className="form-group">
              <label className="form-label">🐄 {T.cattleScore}</label>
              <ScoreSelector value={iCattle} onChange={setICattle} />
            </div>
            <div className="form-group">
              <label className="form-label">{T.paddockCondition}</label>
              <input className="form-input" type="number" min="0" max="100"
                value={iCond} onChange={e=>setICond(e.target.value)} placeholder="0–100"/>
            </div>
            <div className="form-group">
              <label className="form-label">{T.observations}</label>
              <textarea className="form-textarea" rows={2} value={iObs}
                onChange={e=>setIObs(e.target.value)} placeholder="Waarnemings…"/>
            </div>
...
export default function App() {
  ...
  // Animal scores follow animals to destination; paddock condition stays on source
  const applyExtra = (extra, sourcePaddockId, destPaddockId = null) => {
    ...
    if (lostSh+lostGo+lostCa > 0) {
      ...
      addEvent({type:"lost", paddock:sourcePaddockId,
        paddockName:paddocks.find(p=>p.id===sourcePaddockId)?.name,
        sheep:extra.lost.sheep, goats:extra.lost.goats, cattle:extra.lost.cattle});
    }
    ...
    if (deadSh+deadGo+deadCa > 0) {
      ...
      addEvent({type:"death", paddock:sourcePaddockId,
        paddockName:paddocks.find(p=>p.id===sourcePaddockId)?.name,
        sheep:extra.dead.sheep, goats:extra.dead.goats, cattle:extra.dead.cattle, cause:extra.deadCause});
    }
    ...
    if (isSame) {
      if (hasScores || hasPaddock || hasObs) {
        const entry = { id:uid(), ts, source:evtSrc, sheepScore:extra.sheepScore,
          goatScore:extra.goatScore, cattleScore:extra.cattleScore, paddockCond:extra.paddockCond, observations:extra.observations };
        setPaddocks(prev => prev.map(p => p.id===sourcePaddockId
          ? { ...p, paddockCondition:hasPaddock?extra.paddockCond:p.paddockCondition,
                   conditionHistory:[entry,...(p.conditionHistory||[])] } : p));
      }
    } else {
      if (hasScores) {
        const destEntry = { id:uid(), ts, source:evtSrc, sheepScore:extra.sheepScore,
          goatScore:extra.goatScore, cattleScore:extra.cattleScore, paddockCond:null, observations:extra.observations };
        setPaddocks(prev => prev.map(p => p.id===destPaddockId
          ? { ...p, conditionHistory:[destEntry,...(p.conditionHistory||[])] } : p));
      }
      if (hasPaddock || hasObs) {
        const srcEntry = { id:uid(), ts, source:evtSrc, sheepScore:null,
          goatScore:null, paddockCond:extra.paddockCond, observations:extra.observations };
        setPaddocks(prev => prev.map(p => p.id===sourcePaddockId
          ? { ...p, paddockCondition:hasPaddock?extra.paddockCond:p.paddockCondition,
                   conditionHistory:[srcEntry,...(p.conditionHistory||[])] } : p));
      }
    }
  };
    ...
    <>
      ...
      <nav className="nav">
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          <button className="nav-role" onClick={toggleLang} title={T.language}>
            {T.switchLang}
          </button>
          {role==="admin" && (
...
// ── Dashboard ─────────────────────────────────────────────────────────────────
function Dashboard({ paddocks, setPaddocks, events, tasks, issues, role, setTab, addInspection }) {
  const [,] = useLang(); // re-render on language switch
  ...
  return (
    <>
      ...
      {detailId && detailPaddock && (
        <PaddockDetailModal p={detailPaddock} onClose={()=>setDetailId(null)} addInspection={addInspection}/>
      )}
    </>
  );
}
...
// ── Paddock Card ──────────────────────────────────────────────────────────────
function PaddockCard({ p, role, onDelete, onReset, onClick }) {
  const [,] = useLang(); // re-render on language switch
...
// ── Paddocks page ─────────────────────────────────────────────────────────────
function Paddocks({ paddocks, setPaddocks, role, addEvent, addInspection }) {
  const [,] = useLang(); // re-render on language switch
  ...
  const addPaddock = () => {
    ...
    setPaddocks(prev=>[...prev,{id:uid(),name:name.trim(),
      sheep:emptyCat(),goats:emptyCat(),cattle:emptyCattle(),arrivedAt:null,paddockCondition:null,
      conditionHistory:[]}]);
    setName(""); setAddModal(false);
  };

  const openReset = (p) => { setResetP(p); setResetSh({...p.sheep}); setResetGo({...p.goats}); setResetCa({...(p.cattle||emptyCattle())}); };
      ...
      {resetP && (
        <div className="overlay" onClick={()=>setResetP(null)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <CattleCategoryGrid label={`🐄 ${T.cattle}`} values={resetCa}
              onChange={(c,v)=>setResetCa(prev=>({...prev,[c]:Number(v)||0}))}/>
...
function Transaksies({ paddocks, setPaddocks, addEvent }) {
  ...
  const totalAnimals = isNewLambs
    ...
    : totalCat(moveSh)+totalCat(moveGo)+totalCattle(moveCa);
  const valid = paddock && totalAnimals>0;

  const reset = () => {
    setPaddock(""); setMoveSh(emptyCat()); setMoveGo(emptyCat()); setMoveCa(emptyCattle());
  ...
  const submit = () => {
    ...
    if(isNewLambs) {
      ...
      setPaddocks(prev=>prev.map(p=>{
        if(p.id!==paddock) return p;
        const wasEmpty = totalCat(p.sheep)+totalCat(p.goats)===0;
        return {...p, sheep:addCat(p.sheep,moveSh), goats:addCat(p.goats,moveGo), cattle:addCattle(p.cattle||emptyCattle(),moveCa),
      ...
      <div className="card">
        ...
        {isNewLambs ? (
          ...
          <>
            ...
            <CattleCategoryGrid label={`🐄 ${T.cattle}`} values={moveCa}
              onChange={(c,v)=>setMoveCa(prev=>({...prev,[c]:Number(v)||0}))}
              max={type==="sold"?fromP?.cattle||emptyCattle():undefined}/>
          </>
        )}
        <div className="form-group">
  ...
  const submit = () => {
    ...
    for(const c of CATS) {
      if((moveSh[c]||0) > (fromP?.sheep[c]||0))
        return setErr(`${T.errTooManySheep} (${CATLABEL()[c]}): ${T.errOnly} ${fromP?.sheep[c]||0} ${T.errAvail}`);
      if((moveGo[c]||0) > (fromP?.goats[c]||0))
        return setErr(`${T.errTooManyGoats} (${CATLABEL()[c]}): ${T.errOnly} ${fromP?.goats[c]||0} ${T.errAvail}`);
    }
    for(const c of CATTLE_CATS) {
      if((moveCa[c]||0) > ((fromP?.cattle||emptyCattle())[c]||0))
        return setErr(`${T.errTooManyCattle} (${CATTLE_LABEL()[c]}): ${T.errOnly} ${(fromP?.cattle||emptyCattle())[c]||0} ${T.errAvail}`);
    }
    setErr("");
    const ts = nowISO();
    const fromName = paddocks.find(p=>p.id===from)?.name;
    const toName   = paddocks.find(p=>p.id===to)?.name;
    // Main movement — paddockCondition handled by applyExtra on source
    setPaddocks(prev=>prev.map(p=>{
    ...
    addEvent({type:"movement",from,to,fromName,toName,sheep:moveSh,goats:moveGo,cattle:moveCa,notes,
      sheepScore:extra.sheepScore,goatScore:extra.goatScore,
  ...
  return (
    <>
      ...
      <div className="card">
        ...
        <CattleCategoryGrid label={`🐄 ${T.cattle}`} values={moveCa}
          onChange={(c,v)=>setMoveCa(prev=>({...prev,[c]:Number(v)||0}))} max={fromP?.cattle||emptyCattle()}/>
        <div className="form-group">
          <label className="form-label">{T.notes}</label>
      ...
      {fromP && (
        <div className="card" style={{marginTop:10,background:"var(--bg)"}}>
          ...
          <div className="stats">
            ...
            {CATTLE_CATS.map(c=>(
              <div key={c} className="stat">
                <div style={{fontSize:".7rem",color:"var(--muted)"}}>{CATTLE_LABEL()[c]}</div>
                <div className="stat-val" style={{color:"var(--teal)"}}>{(fromP.cattle||emptyCattle())[c]}</div>
              </div>
            ))}
          </div>
        </div>
...
function Verliese({ paddocks, setPaddocks, addEvent }) {
  ...
  const submit = () => {
    ...
    for(const c of CATTLE_CATS) {
      if((cattle[c]||0) > ((fromP?.cattle||emptyCattle())[c]||0))
        return setErr(`${T.errTooManyCattle} (${CATTLE_LABEL()[c]}): ${T.errOnly} ${(fromP?.cattle||emptyCattle())[c]||0} ${T.errAvail}`);
    }
    setErr("");
    const paddockName = paddocks.find(p=>p.id===paddock)?.name;
    setPaddocks(prev=>prev.map(p=>p.id===paddock
      ?{...p,sheep:subCat(p.sheep,sheep),goats:subCat(p.goats,goats),cattle:subCattle(p.cattle||emptyCattle(),cattle)}:p));
    ...
    <>
      ...
      <div className="card">
        ...
        <label className="linked-check">
          <input type="checkbox" checked={linkedToMove}
            onChange={e=>{setLinked(e.target.checked);if(!e.target.checked)setDestPad("");}}/>
          🔀 {T.linkedToMove}
        </label>
        {linkedToMove && (
          <div className="form-group">
              ...
              <div className="card-header">
                <div>
                  ...
                  <div style={{fontSize:".82rem",color:"var(--muted)"}}>
                    {tr.count} × {tr.species==="sheep"?`🐑 ${T.sheep}`:tr.species==="goats"?`🐐 ${T.goats}`:tr.species==="cattle"?`🐄 ${T.cattle}`:T.both} — {CATLABEL()[tr.category]||T.all}
                  </div>
              ...
              {t.status==="pending"&&(
                <div style={{display:"flex",gap:8}}>
                  ...
                  {role==="admin"&&<button className="btn btn-ghost btn-sm" style={{color:"var(--rust)"}}
                    onClick={()=>{if(confirm(T.confirmDeleteTask))setTasks(prev=>prev.filter(x=>x.id!==t.id));}}>❌</button>}
                </div>
              )}
            </div>
          );
            ...
            {iss.status==="open"&&(
              <div style={{display:"flex",gap:8}}>
                ...
                <button className="btn btn-ghost btn-sm" style={{color:"var(--rust)"}}
                  onClick={()=>{if(confirm(T.confirmDeleteIssue))setIssues(prev=>prev.filter(i=>i.id!==iss.id));}}>❌</button>
              </div>
            )}
          </div>
        ))}
            ...
            <div className="form-row">
              <div className="form-group"><label className="form-label">{T.issueType}</label>
                <select className="form-select" value={type} onChange={e=>setType(e.target.value)}>
                  <option value="">{T.generalIssue}</option>
...
const HISTORY_FILTERS = () => [
  {id:"all",    label:T.all},
  {id:"movement",label:T.movements},
  {id:"bought", label:T.bought},
  {id:"sold",   label:T.sold},
  {id:"newLambs",label:T.newLambs},
  {id:"death",  label:T.death},
  {id:"lost",   label:T.lost},
  {id:"treatment",label:T.treatments},
  {id:"task",   label:T.tasks},
  {id:"issue",  label:T.issues},
  {id:"inspection",label:T.inspection},
  {id:"stockTake",label:T.stockTake},
];
...
function Uitvoer({ paddocks, tasks, issues, events, treatments }) {
  ...
  return (
    <>
      ...
      <div className="card" style={{marginBottom:12}}>
        <div className="card-title" style={{marginBottom:10}}>📍 {getLang()==="en"?"Paddock Animals & Condition":"Kamp Diere & Kondisie"}</div>
        <div style={{overflowX:"auto"}}>
        <table className="data-table">
          <thead><tr><th>{T.paddock}</th><th>Tot🐑</th><th>Tot🐐</th><th>Tot🐄</th><th>🐑K</th><th>🐐K</th><th>🐄K</th><th>{T.fieldPaddockCond}</th><th>{T.fieldDays}</th></tr></thead>
          <tbody>{paddocks.map(p=>{
            ...
            return <tr key={p.id}>
              ...
              <td style={{color:latest?.sheepScore!=null?scoreColor(latest.sheepScore):"inherit",fontWeight:700}}>{latest?.sheepScore??"-"}</td>
              <td style={{color:latest?.goatScore!=null?scoreColor(latest.goatScore):"inherit",fontWeight:700}}>{latest?.goatScore??"-"}</td>
              <td style={{color:latest?.cattleScore!=null?scoreColor(latest.cattleScore):"inherit",fontWeight:700}}>{latest?.cattleScore??"-"}</td>
              <td style={{color:p.paddockCondition!=null?(p.paddockCondition>=70?"var(--green)":p.paddockCondition>=40?"var(--amber)":"var(--rust)"):"inherit"}}>
                {p.paddockCondition!=null?`${p.paddockCondition}%`:"-"}
              </td>
      ...
      <div className="card">
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          <button className="btn btn-primary" onClick={()=>dl([
            [T.paddock,`${T.ewes}🐑`,`${T.lambs}🐑`,`${T.rams}🐑`,`${T.wethers}🐑`,`Tot🐑`,`${T.ewes}🐐`,`${T.lambs}🐐`,`${T.rams}🐐`,`${T.wethers}🐐`,`Tot🐐`,`${T.bulls}🐄`,`${T.cows}🐄`,`${T.heifers}🐄`,`${T.calves}🐄`,`Tot🐄`,T.fieldDays,`${T.fieldPaddockCond}%`],
            ...paddocks.map(p=>[p.name,p.sheep.ewes,p.sheep.lambs,p.sheep.rams,p.sheep.wethers,totalCat(p.sheep),
              p.goats.ewes,p.goats.lambs,p.goats.rams,p.goats.wethers,totalCat(p.goats),
              (p.cattle||emptyCattle()).bulls,(p.cattle||emptyCattle()).cows,(p.cattle||emptyCattle()).heifers,(p.cattle||emptyCattle()).calves,totalCattle(p.cattle||emptyCattle()),
              daysSince(p.arrivedAt)??'',p.paddockCondition??''])
          ...
          <button className="btn btn-teal" onClick={()=>dl([
            ...
            ...treatments.map(tr=>[fmtTime(tr.ts),tr.paddockName,
              tr.species==="sheep"?T.sheep:tr.species==="goats"?T.goats:tr.species==="cattle"?T.cattle:T.both,
          ...
          <button className="btn btn-amber" onClick={()=>dl([
            ...
            ...events.map(ev=>{
              ...
              else if(ev.type==="newLambs"){det=`${T.newLambs}: ${ev.sheepLambs||0} ${T.sheepLambsShort}, ${ev.goatLambs||0} ${T.goatLambsShort}`;loc=ev.paddockName||"";}
              else if(ev.type==="death"){det=`${T.dead}: ${totalCat(ev.sheep??{})} ${T.sheep2}, ${totalCat(ev.goats??{})} ${T.goats2}${totalCattle(ev.cattle||emptyCattle())>0?`, ${totalCattle(ev.cattle||emptyCattle())} ${T.cattle2}`:""}${ev.cause?` (${ev.cause})`:""}`;loc=ev.paddockName||"";}
              else if(ev.type==="lost"){det=`${T.missing}: ${totalCat(ev.sheep??{})} ${T.sheep2}, ${totalCat(ev.goats??{})} ${T.goats2}${totalCattle(ev.cattle||emptyCattle())>0?`, ${totalCattle(ev.cattle||emptyCattle())} ${T.cattle2}`:""}`;loc=ev.paddockName||"";}
              else if(ev.type==="treatment"){det=`${ev.treatment||""} — ${ev.count||0} ${getLang()==="en"?"animals":"diere"}${ev.linkedToMove&&ev.destName?` → ${ev.destName}`:""}`;loc=ev.paddockName||"";}
              ...
              else if(ev.type==="task"){det=`${getLang()==="en"?"Task":"Taak"} ${ev.action==="completed"?T.taskCompleted:T.taskAssigned}: ${ev.taskTitle||""}`;loc="";}
              else{det=`${getLang()==="en"?"Issue":"Probleem"} ${ev.action==="resolved"?T.issueResolved:T.issueReported}: ${ev.issueTitle||""}`;loc=ev.paddockName||"";}
...
function EventRow({ ev }) {
  ...
  const configs = {
    ...
    inspection:{icon:"🔍",cls:"ev-insp", title:`${T.inspectionEvent}: ${ev.paddockName||""}`,
      meta:`${ev.sheepScore!==null&&ev.sheepScore!==undefined?` 🐑${ev.sheepScore}/5`:""}${ev.goatScore!==null&&ev.goatScore!==undefined?` 🐐${ev.goatScore}/5`:""}${ev.cattleScore!==null&&ev.cattleScore!==undefined?` 🐄${ev.cattleScore}/5`:""}${ev.paddockCond!=null?` 🌿${ev.paddockCond}%`:""}`.trim()},
