...
// ── Storage (Firestore) ───────────────────────────────────────────────────────
// NOTE: epin key removed — entry gate PIN is gone
const KEYS = { p:"fm4_p", e:"fm4_e", t:"fm4_t", i:"fm4_i", tr:"fm4_tr", pin:"fm4_pin" };

async function load(key, fb) {
  try {
    const snap = await getDoc(doc(db, "farm", key));
    return snap.exists() ? JSON.parse(snap.data().value) : fb;
  } catch { return fb; }
}
async function save(key, val) {
  try { await setDoc(doc(db, "farm", key), { value: JSON.stringify(val) }); } catch {}
}
...
const T = {
  ...
  // Edit tab
  editTab:"Wysig Rekords", editPaddocks:"Kampe", editEvents:"Geskiedenis",
  editTreatments:"Behandelings", editTasks:"Take", editIssues:"Probleme",
  editWarning:"Let op: Verwyder 'n rekord verander NIE kamp getalle outomaties nie. Gebruik 'Stel Nommers' in die Kampe-bladsy om diere te korrigeer.",
  savedEdit:"✅ Gewysig!", deleteConfirm:"Seker jy wil verwyder?",
};
...
// ── Draw sheep icon to canvas, return data URL ────────────────────────────────
function drawSheepIcon(size=192) {
  ...
  c.fillStyle = "#4a5c3a";
  c.beginPath();
  c.moveTo(r, 0); c.lineTo(size-r, 0);
  ...
  // Sheep body — fluffy white cloud of overlapping circles
  c.fillStyle = "#f5f0e8";
  const bodyCircles = [
    [96,98,34],[72,108,26],[120,108,26],[84,84,24],[108,84,24],
    [96,72,22],[68,92,20],[124,92,20],[80,118,20],[112,118,20]
  ];
  bodyCircles.forEach(([x,y,rad]) => {
    c.beginPath();
    c.arc(x*s, y*s, rad*s, 0, Math.PI*2);
    c.fill();
  });
  ...
  c.lineWidth = 2 * s;
  c.beginPath();
  c.ellipse(96*s, 138*s, 18*s, 22*s, 0, 0, Math.PI*2);
  c.fill();
  c.stroke();

  // Eyes
  c.fillStyle = "#2c2416";
  c.beginPath(); c.arc(89*s, 133*s, 3*s, 0, Math.PI*2); c.fill();
  c.beginPath(); c.arc(103*s, 133*s, 3*s, 0, Math.PI*2); c.fill();

  // Nostrils
  c.fillStyle = "#9a7868";
  c.beginPath(); c.ellipse(91*s, 146*s, 2.5*s, 2*s, -0.3, 0, Math.PI*2); c.fill();
  c.beginPath(); c.ellipse(101*s, 146*s, 2.5*s, 2*s, 0.3, 0, Math.PI*2); c.fill();
  ...
  c.lineWidth = 1.5 * s;
  // Left ear
  c.beginPath(); c.ellipse(74*s, 122*s, 7*s, 12*s, -0.5, 0, Math.PI*2);
  c.fill(); c.stroke();
  // Right ear
  c.beginPath(); c.ellipse(118*s, 122*s, 7*s, 12*s, 0.5, 0, Math.PI*2);
  c.fill(); c.stroke();
  ...
  c.lineWidth = 1 * s;
  const legs = [[79,153,8,26],[89,155,8,24],[103,155,8,24],[113,153,8,26]];
  legs.forEach(([x,y,w,h]) => {
    c.beginPath();
    c.roundRect(x*s, y*s, w*s, h*s, 4*s);
    c.fill(); c.stroke();
  });

  // Hooves
  c.fillStyle = "#1a1a1a";
  [[79,173,8,6],[89,173,8,6],[103,173,8,6],[113,173,8,6]].forEach(([x,y,w,h]) => {
    c.beginPath();
    c.roundRect(x*s, y*s, w*s, h*s, 3*s);
    c.fill();
  });
...
// ── Seed data — 31 real paddocks, all starting empty ─────────────────────────
const SEED_PADDOCKS = [
  "Bobos","Middelbos","Kleinbos","Bakenskop","Wallekamp","Masjienkamp",
  "Struiskamp","Leeukuil","Skuurkamp","Oumasjienkamp","Bokskuurkamp",
  "Agterste Lande","Damkamp","Bosand","Ondersand","Kuilekamp","Slagterskamp",
  "Breekdam","Houtwerf","Huiskamp 2","Huiskamp","Perdekamp","Huis Land",
  "Bok lam kampies","Springbok kamp","Langbeen pomp","Langkalk","Middelkalk",
  "Bokalk2","Bokalk 1","Donkiekamp"
].map((name,i)=>({
  id:`sp${i+1}`, name,
  sheep:emptyCat(), goats:emptyCat(),
  arrivedAt:null, paddockCondition:null,
...
.warn-box{background:#fff8e8;border:1.5px solid var(--amber);border-radius:8px;
          padding:10px 13px;font-size:.8rem;color:#7a5a10;margin-bottom:14px;line-height:1.5}
...
.edit-card-row{display:flex;justify-content:space-between;align-items:center;gap:8px}
...
// ══════════════════════════════════════════════════════════════════════════════
//  Role Screen (with admin PIN) — entry gate removed
// ══════════════════════════════════════════════════════════════════════════════
function RoleScreen({ onPick }) {
  ...
  useEffect(()=>{
    load(KEYS.pin, null).then(v => setHasPin(!!v));
  },[]);

  const handleAdmin = async () => {
    const stored = await load(KEYS.pin, null);
    setHasPin(!!stored);
...
export default function App() {
  ...
  // ── Inject sheep icon into document head for PWA / homescreen ──────────────
  useEffect(() => {
    try {
      const iconUrl = drawSheepIcon(192);
      const iconUrl512 = drawSheepIcon(512);
      ...
      // Dynamically patch the PWA manifest so Android also gets the sheep icon
      ...
      themeMeta.content = "#4a5c3a";

      // App title
      document.title = "Plaasbestuurder";
    } catch(e) {
      console.warn("Icon inject failed", e);
    }
  }, []);

  useEffect(() => {
    (async () => {
      const [p,e,t,i,tr] = await Promise.all([
        load(KEYS.p, SEED_PADDOCKS), load(KEYS.e,[]), load(KEYS.t,[]),
        load(KEYS.i,[]), load(KEYS.tr,[]),
      ]);
      setPaddocks(p);setEvents(e);setTasks(t);setIssues(i);setTreatments(tr);setLoaded(true);
    })();
  }, []);

  useEffect(() => { if(loaded){ setSyncing(true); save(KEYS.p, paddocks).then(()=>setSyncing(false)); }}, [paddocks, loaded]);
  useEffect(() => { if(loaded) save(KEYS.e, events);     }, [events,     loaded]);
  useEffect(() => { if(loaded) save(KEYS.t, tasks);      }, [tasks,      loaded]);
  useEffect(() => { if(loaded) save(KEYS.i, issues);     }, [issues,     loaded]);
  useEffect(() => { if(loaded) save(KEYS.tr,treatments); }, [treatments, loaded]);
  ...
  const handleNavSwitch = async () => {
    if (role === "admin") {
      ...
      setPinVal(""); setPinErr(""); setPinModal(true);
    }
  };
  ...
  if (!loaded) return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100vh",background:"var(--olive)",color:"#fff",gap:12}}>
      <div style={{fontSize:"3rem"}}>🐑</div>
      <div style={{fontFamily:"'Playfair Display',serif",fontSize:"1.4rem"}}>Plaasbestuurder</div>
      <div style={{fontSize:".85rem",opacity:.7}}>Laai data van bediener…</div>
    </div>
  );

  if (!role) return <><style>{CSS}</style><RoleScreen onPick={r=>setRole(r)}/></>;

  const pendingCount = tasks.filter(t=>t.status==="pending").length;
  const overdueCount = tasks.filter(t=>t.status==="pending"&&isOverdue(t.dueDate)).length;
  const openIssues   = issues.filter(i=>i.status==="open").length;

  // Edit tab only shown to admin
  const TABS = [
    ...
    ...(role==="admin"?[{id:"edit", label:"✏️ "+T.edit}]:[]),
  ];
...
// ── Wysig (Edit) ──────────────────────────────────────────────────────────────
function Wysig({ paddocks, setPaddocks, events, setEvents, tasks, setTasks,
                  issues, setIssues, treatments, setTreatments, addEvent }) {
  const [section, setSection] = useState("kampe");
  ...
  const SECTIONS = [
    ...
    {id:"geskiedenis",label:"📋 Geskiedenis"},
    {id:"behandelings",label:"💊 Behandelings"},
    {id:"take",       label:"✅ Take"},
    {id:"probleme",   label:"⚠️ Probleme"},
  ];

  return (
    <>
      <div className="sec-head"><h2>✏️ {T.editTab}</h2></div>
      ...
      {flash && (
        <div style={{background:"#e8f4e8",border:"1.5px solid #a8d4a8",borderRadius:8,
          padding:"9px 13px",marginBottom:12,fontWeight:600,color:"var(--green)",fontSize:".88rem"}}>
          {flash}
        </div>
      )}
      ...
      <div style={{display:"flex",gap:6,marginBottom:16,flexWrap:"wrap"}}>
        {SECTIONS.map(s=>(
          <button key={s.id} className={`btn btn-sm ${section===s.id?"btn-primary":"btn-ghost"}`}
      ...
      {section==="kampe" && (
        <div>
          <div style={{fontSize:".82rem",color:"var(--muted)",marginBottom:12}}>
            Tik op 'n kamp om sy diere-getalle direk te wysig.
          </div>
          {paddocks.map(p => (
            <EditPaddockCard key={p.id} p={p}
              onSave={(sh,go) => {
                setPaddocks(prev=>prev.map(x=>x.id===p.id ? {...x,sheep:sh,goats:go,
                  arrivedAt:(totalCat(sh)+totalCat(go)>0)?(x.arrivedAt||nowISO()):null} : x));
                addEvent({type:"stockTake",paddock:p.id,paddockName:p.name,
                  before:{sheep:{...p.sheep},goats:{...p.goats}},after:{sheep:sh,goats:go}});
                showFlash(`✅ ${p.name} gewysig!`);
              }}
            />
          ))}
        </div>
      ...
      {section==="geskiedenis" && (
        <div>
          <div style={{fontSize:".82rem",color:"var(--muted)",marginBottom:12}}>
            Verwyder foutiewe geskiedenisrekords. Kamp getalle word NIE outomaties aangepas nie.
          </div>
          {events.length===0 && <div className="empty"><div className="empty-icon">📋</div><p>{T.noEvents}</p></div>}
          {events.map(ev=>(
            <div className="edit-card" key={ev.id}>
              <div className="edit-card-row">
                <div style={{flex:1}}>
                  <div style={{fontWeight:600,fontSize:".88rem"}}>{eventLabel(ev)}</div>
                  <div style={{fontSize:".75rem",color:"var(--muted)",marginTop:2}}>{fmtTime(ev.ts)}</div>
                </div>
                <button className="btn btn-ghost btn-sm" style={{color:"var(--rust)",flexShrink:0}}
                  onClick={()=>{if(confirm(T.deleteConfirm))setEvents(prev=>prev.filter(e=>e.id!==ev.id));}}>
                  🗑️
                </button>
              </div>
            </div>
      ...
      {/* ── BEHANDELINGS ── */}
      {section==="behandelings" && (
        <div>
          {treatments.length===0 && <div className="empty"><div className="empty-icon">💊</div><p>{T.noTreatments}</p></div>}
          {treatments.map(tr=>(
            <div className="edit-card" key={tr.id}>
              <div className="edit-card-row">
                <div style={{flex:1}}>
                  <div style={{fontWeight:600,fontSize:".88rem"}}>💊 {tr.treatment}</div>
                  <div style={{fontSize:".8rem",color:"var(--muted)"}}>
                    {tr.count} diere · {tr.paddockName} · {fmtTime(tr.ts)}
                  </div>
                  {tr.dosage && <div style={{fontSize:".78rem",color:"var(--muted)"}}>{T.dosage}: {tr.dosage}</div>}
                </div>
                <div style={{display:"flex",gap:6,flexShrink:0}}>
                  <button className="btn btn-ghost btn-sm"
                    onClick={()=>setEditModal({type:"treatment",item:{...tr}})}>✏️</button>
                  <button className="btn btn-ghost btn-sm" style={{color:"var(--rust)"}}
                    onClick={()=>{if(confirm(T.deleteConfirm))setTreatments(prev=>prev.filter(t=>t.id!==tr.id));}}>🗑️</button>
                </div>
              </div>
            </div>
          ))}
      ...
      {section==="take" && (
        <div>
          {tasks.length===0 && <div className="empty"><div className="empty-icon">✅</div><p>{T.noTasks}</p></div>}
          {tasks.map(t=>(
            <div className="edit-card" key={t.id}>
              <div className="edit-card-row">
                <div style={{flex:1}}>
                  <div style={{fontWeight:600,fontSize:".88rem"}}>{t.title}</div>
                  <div style={{fontSize:".8rem",color:"var(--muted)"}}>
                    {t.assignedTo?`→ ${t.assignedTo} · `:""}
                    {t.dueDate?`Sperdatum: ${fmtDate(t.dueDate)} · `:""}
                    <span className={`pill ${t.status==="done"?"pill-done":isOverdue(t.dueDate)&&t.status==="pending"?"pill-overdue":"pill-pending"}`}
                      style={{fontSize:".68rem"}}>
                      {t.status==="done"?"✅ Klaar":isOverdue(t.dueDate)&&t.status==="pending"?"⚠️ Agterstallig":"⏳ Hangende"}
                    </span>
                  </div>
                </div>
                <div style={{display:"flex",gap:6,flexShrink:0}}>
                  <button className="btn btn-ghost btn-sm"
                    onClick={()=>setEditModal({type:"task",item:{...t}})}>✏️</button>
                  <button className="btn btn-ghost btn-sm" style={{color:"var(--rust)"}}
                    onClick={()=>{if(confirm(T.deleteConfirm))setTasks(prev=>prev.filter(x=>x.id!==t.id));}}>🗑️</button>
                </div>
              </div>
            </div>
          ))}
      ...
      {section==="probleme" && (
        <div>
          {issues.length===0 && <div className="empty"><div className="empty-icon">⚠️</div><p>{T.noIssues}</p></div>}
          {issues.map(iss=>(
            <div className="edit-card" key={iss.id}>
              <div className="edit-card-row">
                <div style={{flex:1}}>
                  <div style={{fontWeight:600,fontSize:".88rem"}}>{iss.title}</div>
                  <div style={{fontSize:".8rem",color:"var(--muted)"}}>
                    {iss.type?`${iss.type} · `:""}
                    <span className={`pill ${iss.status==="open"?"pill-open":"pill-resolved"}`} style={{fontSize:".68rem"}}>
                      {iss.status==="open"?"🔴 Oop":"✅ Opgelos"}
                    </span>
                  </div>
                </div>
                <div style={{display:"flex",gap:6,flexShrink:0}}>
                  <button className="btn btn-ghost btn-sm"
                    onClick={()=>setEditModal({type:"issue",item:{...iss}})}>✏️</button>
                  <button className="btn btn-ghost btn-sm" style={{color:"var(--rust)"}}
                    onClick={()=>{if(confirm(T.deleteConfirm))setIssues(prev=>prev.filter(i=>i.id!==iss.id));}}>🗑️</button>
                </div>
              </div>
            </div>
          ))}
...
// ── Edit Paddock Card (inline expand) ─────────────────────────────────────────
function EditPaddockCard({ p, onSave }) {
  const [open,   setOpen]   = useState(false);
  const [editSh, setEditSh] = useState({...p.sheep});
  const [editGo, setEditGo] = useState({...p.goats});

  const handleOpen = () => {
    setEditSh({...p.sheep}); setEditGo({...p.goats}); setOpen(true);
  };

  return (
    <div className="edit-card" style={{marginBottom:8}}>
      <div className="edit-card-row" style={{cursor:"pointer"}} onClick={()=>open?setOpen(false):handleOpen()}>
        <div style={{flex:1}}>
          <div style={{fontWeight:600,fontSize:".9rem"}}>{p.name}</div>
          <div style={{fontSize:".78rem",color:"var(--muted)",marginTop:2}}>
            🐑 {totalCat(p.sheep)} skape &nbsp;·&nbsp; 🐐 {totalCat(p.goats)} bokke
          </div>
        </div>
        <span style={{color:"var(--muted)",fontSize:"1.1rem"}}>{open?"▲":"✏️"}</span>
      </div>
      {open && (
        <div style={{marginTop:12,borderTop:"1px solid var(--border)",paddingTop:12}}>
          <CategoryGrid label={`🐑 ${T.sheep}`} values={editSh}
            onChange={(c,v)=>setEditSh(prev=>({...prev,[c]:Number(v)||0}))}/>
          <CategoryGrid label={`🐐 ${T.goats}`} values={editGo}
            onChange={(c,v)=>setEditGo(prev=>({...prev,[c]:Number(v)||0}))}/>
          <div style={{display:"flex",gap:8,marginTop:4}}>
            <button className="btn btn-primary btn-sm" style={{flex:1}}
              onClick={()=>{ onSave({...editSh},{...editGo}); setOpen(false); }}>
              {T.save}
            </button>
...
// ── Edit Modal (treatments, tasks, issues) ────────────────────────────────────
function EditModal({ data, paddocks, onClose, onSave }) {
  const [item, setItem] = useState(data.item);
  const upd = (k,v) => setItem(prev=>({...prev,[k]:v}));
  ...
  return (
    <div className="overlay" onClick={onClose}>
      <div className="modal" onClick={e=>e.stopPropagation()}>
        ...
        {data.type==="treatment" && <>
          <div className="form-group"><label className="form-label">{T.treatmentName}</label>
            <input className="form-input" value={item.treatment||""} onChange={e=>upd("treatment",e.target.value)}/></div>
          <div className="form-row">
            <div className="form-group"><label className="form-label">{T.dosage}</label>
              <input className="form-input" value={item.dosage||""} onChange={e=>upd("dosage",e.target.value)}/></div>
            <div className="form-group"><label className="form-label">{T.countTreated}</label>
              <input className="form-input" type="number" min="1" value={item.count||""} onChange={e=>upd("count",Number(e.target.value))}/></div>
          </div>
          <div className="form-group"><label className="form-label">{T.administeredBy}</label>
            <input className="form-input" value={item.administeredBy||""} onChange={e=>upd("administeredBy",e.target.value)}/></div>
          <div className="form-group"><label className="form-label">{T.notes}</label>
            <textarea className="form-textarea" rows={2} value={item.notes||""} onChange={e=>upd("notes",e.target.value)}/></div>
        </>}

        {data.type==="task" && <>
          <div className="form-group"><label className="form-label">{T.taskDesc}</label>
            <input className="form-input" value={item.title||""} onChange={e=>upd("title",e.target.value)}/></div>
          <div className="form-row">
            <div className="form-group"><label className="form-label">{T.assignTo}</label>
              <input className="form-input" value={item.assignedTo||""} onChange={e=>upd("assignedTo",e.target.value)}/></div>
            <div className="form-group"><label className="form-label">{T.dueDate}</label>
              <input className="form-input" type="date" value={item.dueDate||""} onChange={e=>upd("dueDate",e.target.value)}/></div>
          </div>
          <div className="form-group"><label className="form-label">{T.paddock}</label>
            <select className="form-select" value={item.paddock||""} onChange={e=>upd("paddock",e.target.value)}>
              <option value="">{T.noSpecific}</option>
              {paddocks.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}
            </select></div>
          <div className="form-group"><label className="form-label">{T.notes}</label>
          ...
          <div className="form-group"><label className="form-label">Status</label>
            <select className="form-select" value={item.status||"pending"} onChange={e=>upd("status",e.target.value)}>
        ...
        {data.type==="issue" && <>
          ...
          <div className="form-row">
            <div className="form-group"><label className="form-label">{T.issueType}</label>
              <select className="form-select" value={item.type||""} onChange={e=>upd("type",e.target.value)}>
                <option value="">Algemeen</option>
                <option value={T.fenceGate}>{T.fenceGate}</option>
                <option value={T.waterSupply}>{T.waterSupply}</option>
                <option value={T.animalHealth}>{T.animalHealth}</option>
          ...
          <div className="form-group"><label className="form-label">{T.notes}</label>
            <textarea className="form-textarea" rows={2} value={item.notes||""} onChange={e=>upd("notes",e.target.value)}/></div>
          <div className="form-group"><label className="form-label">Status</label>
            <select className="form-select" value={item.status||"open"} onChange={e=>upd("status",e.target.value)}>
        ...
        <div style={{display:"flex",gap:8,marginTop:4}}>
          <button className="btn btn-primary" style={{flex:1}} onClick={()=>onSave(item)}>{T.save}</button>
...
function eventLabel(ev) {
  if(ev.type==="movement")  return `🔀 Geskuif: ${ev.fromName||""}→${ev.toName||""} (${totalCat(ev.sheep??{})+totalCat(ev.goats??{})} diere)`;
  if(ev.type==="bought")    return `🛒 Gekoop: ${totalCat(ev.sheep??{})+totalCat(ev.goats??{})} diere → ${ev.paddockName||""}`;
  if(ev.type==="sold")      return `💰 Verkoop: ${totalCat(ev.sheep??{})+totalCat(ev.goats??{})} diere uit ${ev.paddockName||""}`;
  if(ev.type==="newLambs")  return `🐣 Nuwe Lammers: ${(ev.sheepLambs||0)+(ev.goatLambs||0)} in ${ev.paddockName||""}`;
  if(ev.type==="death")     return `☠️ Sterfte: ${totalCat(ev.sheep??{})+totalCat(ev.goats??{})} in ${ev.paddockName||""}`;
  if(ev.type==="lost")      return `🔍 Vermis: ${totalCat(ev.sheep??{})+totalCat(ev.goats??{})} in ${ev.paddockName||""}`;
  if(ev.type==="treatment") return `💊 Behandeling: ${ev.treatment||""} — ${ev.count||0} diere in ${ev.paddockName||""}`;
  if(ev.type==="stockTake") return `📊 Voorraadopname: ${ev.paddockName||""}`;
  if(ev.type==="task")      return `✅ Taak ${ev.action==="completed"?"voltooi":"toegewys"}: ${ev.taskTitle||""}`;
  if(ev.type==="issue")     return `⚠️ Probleem ${ev.action==="resolved"?"opgelos":"gerapporteer"}: ${ev.issueTitle||""}`;
  return ev.type;
}
