...
async function save(key, val, farmId, force = false) {
  ...
  // Pass force=true to bypass (e.g. intentional farm reset)
...
const TRANSLATIONS = {
  af: {
    ...
    ewes:"Ooie", lambs:"Lammers", rams:"Ramme", wethers:"Hamels", goatWethers:"Kapaters", total:"Totaal",
    ...
    bought:"Gekoop", sold:"Verkoop", newLambs:"Aanteel",
    stockTake:"Voorraadopname", setNumbers:"Stel Nommers",
    transactionSaved:"✅ Transaksie Rekordeer!", recordTransaction:"Rekordeer →",
    sheepLambs:"Skaaplammers", goatLambs:"Boklammers", newCalves:"Nuwe Kalwers", cattleCalves:"Beeskalwers",
    ...
    treatmentsLbl:"Behandelings", inspections:"Inspeksies", pendingTasksLbl:"Hangende Take",
    ...
    searchPaddocks:"🔍 Soek kampe...", noMatches:"Geen kampe pas hierdie soektog nie",
    overduePrefix:"Agterstallig — was gesperd", dueDatePrefix:"Sperdatum:",
    resolvedAt:"Opgelos", doneAt:"Klaar",
    workerNamePlaceholder:"Werker naam", taskNotesPlaceholder:"Ekstra besonderhede…",
    issueNotesPlaceholder:"Meer besonderhede…", observationsPlaceholder:"Enige waarnemings…",
    notesPlaceholder:"Notas…", newPaddockPlaceholder:"bv. Westelike Kamp",
    adminByPlaceholder:"Naam…", taskTitlePlaceholder:"bv. Kontroleer watertrôe",
    issueDescPlaceholder:"bv. Heining gebreek",
  },
  en: {
    ...
    ewes:"Ewes", lambs:"Lambs", rams:"Rams", wethers:"Wethers", goatWethers:"Wethers", total:"Total",
    ...
    bought:"Bought", sold:"Sold", newLambs:"Increase",
    stockTake:"Stock Take", setNumbers:"Set Numbers",
    transactionSaved:"✅ Transaction Recorded!", recordTransaction:"Record →",
    sheepLambs:"Sheep Lambs", goatLambs:"Goat Lambs", newCalves:"New Calves", cattleCalves:"Cattle Calves",
    ...
    treatmentsLbl:"Treatments", inspections:"Inspections", pendingTasksLbl:"Pending Tasks",
    ...
    searchPaddocks:"🔍 Search paddocks...", noMatches:"No paddocks match that search",
    overduePrefix:"Overdue — was due", dueDatePrefix:"Due date:",
    resolvedAt:"Resolved", doneAt:"Done",
    workerNamePlaceholder:"Worker name", taskNotesPlaceholder:"Extra details…",
    issueNotesPlaceholder:"More details…", observationsPlaceholder:"Any observations…",
    notesPlaceholder:"Notes…", newPaddockPlaceholder:"e.g. Western Paddock",
    adminByPlaceholder:"Name…", taskTitlePlaceholder:"e.g. Check water troughs",
    issueDescPlaceholder:"e.g. Fence broken",
  }
};
...
const GOAT_CATLABEL = () => ({ ewes:T.ewes, lambs:T.lambs, rams:T.rams, wethers:T.goatWethers });
...
// Events are stored newest-first, but must be reduced oldest-first: each
// subtraction is clamped at 0 (counts can't go negative), so processing a
// "found" event before the "lost" event it offsets would zero it out
// instead of cancelling it, leaving a phantom missing animal forever.
function computeMissingTotals(events) {
  let sheep = emptyCat(), goats = emptyCat(), cattle = emptyCattle();
  [...events].sort((a,b)=>new Date(a.ts||0)-new Date(b.ts||0)).forEach(ev => {
    if (ev.type === "lost") {
...
// Alphabetic ordering for paddock dropdowns (case-insensitive, numeric-aware so "Kamp 2" < "Kamp 10")
const byPaddockName  = (a,b) => (a.name||"").localeCompare(b.name||"", undefined, {sensitivity:"base", numeric:true});
const sortedPaddocks = (arr) => [...arr].sort(byPaddockName);
...
.tabs{display:flex;flex-wrap:wrap;gap:5px;background:var(--olive);padding:8px 10px 10px}
.tab{background:transparent;border:none;color:rgba(255,255,255,.6);padding:6px 10px;
     border-radius:8px;font-size:.74rem;font-weight:600;white-space:nowrap;transition:all .2s}
.tab.active{background:var(--bg);color:var(--olive)}
.tab:hover:not(.active){background:rgba(255,255,255,.12);color:#fff}

.page{padding:14px 12px;max-width:680px;margin:0 auto}
...
.species-block{background:var(--bg);border-radius:10px;padding:10px;margin-bottom:8px}
.species-block h4{font-size:.8rem;font-weight:700;margin-bottom:6px;color:var(--olive)}
...
.paddock-row{display:flex;align-items:center;gap:8px;padding:11px 4px;border-bottom:1px solid var(--border);cursor:pointer}
.paddock-row:last-child{border-bottom:none}
.paddock-row:active{background:var(--bg)}
.paddock-row.is-resting{opacity:.6}
.paddock-row-name{flex:1;min-width:0;font-weight:700;font-size:.88rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;display:flex;align-items:center;gap:4px}
.paddock-row-badges{display:flex;gap:7px;flex-shrink:0;font-size:.74rem;font-weight:700}
.paddock-row-lsu{flex-shrink:0;font-size:.76rem;font-weight:700;min-width:34px;text-align:right}
.paddock-row-days{flex-shrink:0;font-size:.72rem;font-weight:700}
.paddock-row-arrow{flex-shrink:0;color:var(--border);font-size:1.05rem}
...
.extra-section{background:var(--bg);border-radius:10px;padding:10px;margin-top:4px}
.extra-section h4{font-size:.8rem;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:8px}
...
function GoatCategoryGrid({ label, values, onChange, max }) {
  const [,] = useLang();
  return (
    <div className="species-block">
      <h4>{label}</h4>
      <div className="cat-grid">
        {CATS.map(c => (
          <div key={c} className="form-group" style={{marginBottom:0}}>
            <label className="form-label">{GOAT_CATLABEL()[c]}</label>
            <input className="form-input" type="number" min="0"
              max={max?.[c] ?? undefined}
              value={values[c] || ""}
              onChange={e => onChange(c, e.target.value)}
    ...
    <>
      ...
      {open && (
        <div className="extra-section" style={{marginTop:8}}>
          ...
          <GoatCategoryGrid label={`🐐 ${T.goats}`} values={extra.lost.goats} onChange={(c,v)=>upd(`lost.goats.${c}`,Number(v)||0)} />
          ...
          <GoatCategoryGrid label={`🐐 ${T.goats}`} values={extra.dead.goats} onChange={(c,v)=>upd(`dead.goats.${c}`,Number(v)||0)} />
          ...
          <div className="form-group">
            ...
            <textarea className="form-textarea" rows={2} value={extra.observations}
              onChange={e=>upd("observations",e.target.value)} placeholder={T.observationsPlaceholder}/>
          </div>
        </div>
      )}
    </>
...
// ══════════════════════════════════════════════════════════════════════════════
function PaddockDetailModal({ p, settings, onClose, addInspection, role, onReset, onRestManage, onDelete }) {
  const [,] = useLang();
  ...
  return (
    <div className="overlay" onClick={onClose}>
      <div className="modal" onClick={e=>e.stopPropagation()}>
        ...
        {role==="admin" && (onReset||onRestManage||onDelete) && (
          <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:14}}>
            {onReset      && <button className="btn btn-ghost btn-sm" onClick={()=>{onClose();onReset();}}>📊 {T.setNumbers}</button>}
            {onRestManage && <button className="btn btn-ghost btn-sm" onClick={()=>{onClose();onRestManage();}}>🌱 {T.restPlan}</button>}
            {onDelete     && <button className="btn btn-ghost btn-sm" style={{color:"var(--rust)"}} onClick={()=>{if(confirm(`${p.name} ${T.confirmDelete}`)){onClose();onDelete();}}}>🗑️ {T.delete}</button>}
          </div>
        )}
        {days!==null && (hasSheep||hasGoats||totalCattle(p.cattle||emptyCattle())>0) && (
          <div className={`graze-badge ${gradeClass}`} style={{marginBottom:12}}>🌿 {gradeLabel}</div>
        )}
        ...
        {hasGoats && (
          <div style={{marginBottom:10}}>
            ...
            <div className="stats">
              {CATS.map(c=>(<div key={c} className="stat"><div className="stat-val goats-c">{p.goats[c]}</div><div className="stat-lbl">{GOAT_CATLABEL()[c]}</div></div>))}
        ...
        {showInsp && (
          <div className="extra-section" style={{marginTop:8}}>
            ...
            <div className="form-group">
              <label className="form-label">{T.observations}</label>
              <textarea className="form-textarea" rows={2} value={iObs} onChange={e=>setIObs(e.target.value)} placeholder={T.observationsPlaceholder}/>
            </div>
...
export default function App() {
  ...
  const handleFarmReset = async (confirmedName) => {
    ...
    if (farmId) {
      await Promise.all([
        save(KEYS.p,  [], farmId, true),
        save(KEYS.e,  [], farmId, true),
        save(KEYS.t,  [], farmId, true),
        save(KEYS.i,  [], farmId, true),
        save(KEYS.tr, [], farmId, true),
      ]);
    }
    return true;
  };
  ...
  const applyExtra = (extra, sourcePaddockId, destPaddockId = null, ts = null) => {
    ...
    if (isSame) {
      ...
      if (hasPaddock || hasObs) {
        const srcEntry = { id:uid(), ts:evTs, source:evtSrc, sheepScore:null, goatScore:null, cattleScore:null, paddockCond:extra.paddockCond, observations:extra.observations };
        setPaddocks(prev => prev.map(p => p.id===sourcePaddockId
  ...
  return (
    <>
      ...
      <div className="page">
        {tab==="dashboard"    && <Dashboard    {...ctx} setTab={setTab} farmTotalLSU={farmTotalLSU} farmLoadPct={farmLoadPct}/>}
        ...
        {tab==="history"      && <Geskiedenis  events={events} setEvents={setEvents} paddocks={paddocks} role={role} pushUndo={pushUndo} snapNow={snapNow} />}
        ...
        <div style={{borderTop:"1px solid var(--border)",marginTop:16,paddingTop:16}}>
          ...
          <div className="form-group">
            ...
            <input className="form-input" type="number" min="0.1" step="0.1" value={s.haPerLSU}
              onChange={e=>upd("haPerLSU",e.target.value)} placeholder={getLang()==="en"?"e.g. 8":"bv. 8"}/>
...
// ── Dashboard ─────────────────────────────────────────────────────────────────
function Dashboard({ paddocks, setPaddocks, events, tasks, issues, role, setTab, addInspection, settings, farmTotalLSU, farmLoadPct, pushUndo, snapNow }) {
  const [,] = useLang();
  ...
  const deletePaddock = (id) => {
    pushUndo(getLang()==="en"?"Paddock deleted":"Kamp verwyder", snapNow());
    setPaddocks(prev=>prev.filter(x=>x.id!==id));
  };
    ...
    <>
      ...
      <div className="summary-strip">
        ...
        <div className="chip">
          ...
          <div className="chip-lbl">{overdue?`⚠️ ${overdue} ${T.overdue}`:T.pendingTasks}</div>
        </div>
      ...
      <div className="sec-head" style={{marginTop:8}}>
        <h2>{T.paddocks}</h2>
        <span style={{fontSize:".72rem",color:"var(--muted)"}}>{getLang()==="en"?"Tap for detail":"Tik vir detail"}</span>
      </div>
      {(() => {
        const stocked = paddocks.filter(p =>
          totalCat(p.sheep) + totalCat(p.goats) + totalCattle(p.cattle || emptyCattle()) > 0
        );
        ...
        if (stocked.length === 0) {
          return (
            <div className="empty">
              <div className="empty-icon">📍</div>
              <p>{getLang()==="en" ? "No paddocks have animals yet" : "Geen kampe het nog diere nie"}</p>
            </div>
          );
        }
        const shown = sortedPaddocks(stocked).filter(p => p.name.toLowerCase().includes(search.trim().toLowerCase()));
        return (
          <>
            {stocked.length > 5 && (
              <input className="form-input search-bar" value={search} onChange={e=>setSearch(e.target.value)} placeholder={T.searchPaddocks}/>
            )}
            {shown.length === 0 ? (
              <div className="empty"><div className="empty-icon">🔍</div><p>{T.noMatches}</p></div>
            ) : (
              <div className="card" style={{padding:"4px 13px"}}>
                {shown.map(p=>(
                  <PaddockRow key={p.id} p={p} settings={settings} onClick={()=>setDetailId(p.id)}/>
                ))}
              </div>
            )}
          </>
      ...
      {detailId && detailPaddock && (
        <PaddockDetailModal p={detailPaddock} settings={settings} onClose={()=>setDetailId(null)} addInspection={addInspection}
          role={role} onDelete={role==="admin" ? ()=>deletePaddock(detailPaddock.id) : undefined}/>
      )}
    </>
  );
}

// ── Paddock Row (compact list item) ────────────────────────────────────────────
function PaddockRow({ p, settings, onClick }) {
  const [,] = useLang();
  const hasSheep  = totalCat(p.sheep)>0;
  const hasGoats  = totalCat(p.goats)>0;
  const hasCattle = totalCattle(p.cattle||emptyCattle())>0;
  const hasAnimals = hasSheep||hasGoats||hasCattle;
  const days = daysSince(p.arrivedAt);
  const dayColor = days===null?null:days<7?"var(--green)":days<14?"var(--amber)":"var(--rust)";
  const lsu = settings ? calcLSU(p, settings) : null;
  const lsuPct = lsu?.loadPct;
  const lsuColor = lsuPct==null ? null : lsuPct>100 ? "var(--rust)" : lsuPct>80 ? "var(--amber)" : "var(--green)";

  return (
    <div className={`paddock-row ${p.resting?"is-resting":""}`} onClick={onClick}>
      <div className="paddock-row-name">
        {p.resting && <span title={T.resting}>🌱</span>}
        <span>{p.name}</span>
      </div>
      {hasAnimals && days!==null && <span className="paddock-row-days" style={{color:dayColor}} title={`${days} ${T.daysInPaddock}`}>🌿{days}d</span>}
      <div className="paddock-row-badges">
        {hasSheep  && <span style={{color:"var(--olive)"}}>🐑{totalCat(p.sheep)}</span>}
        {hasGoats  && <span style={{color:"var(--amber)"}}>🐐{totalCat(p.goats)}</span>}
        {hasCattle && <span style={{color:"var(--teal)"}}>🐄{totalCattle(p.cattle||emptyCattle())}</span>}
        {!hasAnimals && <span style={{color:"var(--muted)"}}>—</span>}
      </div>
      {lsuPct!=null && <div className="paddock-row-lsu" style={{color:lsuColor}}>{lsuPct.toFixed(0)}%</div>}
      <span className="paddock-row-arrow">›</span>
    </div>
  );
}
...
function Paddocks({ paddocks, setPaddocks, role, addEvent, addInspection, settings, pushUndo, snapNow }) {
  ...
  const saveRestSettings = (paddockId, updates) => {
    ...
    if (willClearRest) addEvent({ type:"restEnd", ts:nowISO(), paddock:paddockId, paddockName:current.name });
  };
  ...
  return (
    <>
      ...
      {paddocks.length>0 && (() => {
        const list = sortedPaddocks(paddocks).filter(p => p.name.toLowerCase().includes(search.trim().toLowerCase()));
        return (
          <>
            {paddocks.length > 5 && (
              <input className="form-input search-bar" value={search} onChange={e=>setSearch(e.target.value)} placeholder={T.searchPaddocks}/>
        ...
        <div className="overlay" onClick={()=>setAddModal(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="form-group">
              ...
              <input className="form-input" value={name} onChange={e=>setName(e.target.value)} placeholder={T.newPaddockPlaceholder} autoFocus/>
            </div>
            <div className="form-group">
              <label className="form-label">{T.hectares}</label>
              <input className="form-input" type="number" min="0" step="0.1" value={hectares}
                onChange={e=>setHectares(e.target.value)} placeholder={getLang()==="en"?"e.g. 45":"bv. 45"}/>
          ...
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="form-group">
              ...
              <input className="form-input" type="number" min="0" step="0.1" value={resetHa}
                onChange={e=>setResetHa(e.target.value)} placeholder={getLang()==="en"?"e.g. 45":"bv. 45"}/>
            </div>
            <CategoryGrid label={`🐑 ${T.sheep}`} values={resetSh} onChange={(c,v)=>setResetSh(prev=>({...prev,[c]:Number(v)||0}))}/>
            <GoatCategoryGrid label={`🐐 ${T.goats}`} values={resetGo} onChange={(c,v)=>setResetGo(prev=>({...prev,[c]:Number(v)||0}))}/>
      ...
      {detailId && detailPaddock && (
        <PaddockDetailModal p={detailPaddock} settings={settings} onClose={()=>setDetailId(null)} addInspection={addInspection}
          role={role}
          onReset={()=>openReset(detailPaddock)}
          onRestManage={()=>setRestModalP(detailPaddock)}
          onDelete={()=>{ pushUndo(getLang()==="en"?"Paddock deleted":"Kamp verwyder", snapNow()); setPaddocks(prev=>prev.filter(x=>x.id!==detailPaddock.id)); }}
        />
      )}
      {restModalP && (
...
function Transaksies({ paddocks, setPaddocks, addEvent, settings, pushUndo, snapNow }) {
  ...
  const [cattleCalves, setCattleCalves] = useState("");
  ...
  const totalAnimals = isNewLambs ? (Number(sheepLambs)||0)+(Number(goatLambs)||0)+(Number(cattleCalves)||0) : totalCat(moveSh)+totalCat(moveGo)+totalCattle(moveCa);
  const valid = paddock && totalAnimals>0;

  const reset = () => { setPaddock(""); setMoveSh(emptyCat()); setMoveGo(emptyCat()); setMoveCa(emptyCattle()); setSheepLambs(""); setGoatLambs(""); setCattleCalves(""); setNotes(""); setEventDate(todayStr()); };
  ...
  const submit = () => {
    ...
    if(isNewLambs) {
      const sl = Number(sheepLambs)||0, gl = Number(goatLambs)||0, cc = Number(cattleCalves)||0;
      setPaddocks(prev=>prev.map(p=>{
        if(p.id!==paddock) return p;
        const newSh = {...p.sheep, lambs:(p.sheep.lambs||0)+sl};
        const newGo = {...p.goats, lambs:(p.goats.lambs||0)+gl};
        const newCa = {...(p.cattle||emptyCattle()), calves:((p.cattle||emptyCattle()).calves||0)+cc};
        const newLSU = calcCurrentLSU({...p,sheep:newSh,goats:newGo,cattle:newCa}, settings).totalLSU;
        const wasEmpty = totalCat(p.sheep)+totalCat(p.goats)+totalCattle(p.cattle||emptyCattle())===0;
        const updated = wasEmpty && (sl+gl+cc)>0
      ...
      addEvent({type:"newLambs", ts, paddock, paddockName, sheepLambs:sl, goatLambs:gl, cattleCalves:cc, notes});
    } else if(type==="bought") {
      setPaddocks(prev=>prev.map(p=>{
        if(p.id!==paddock) return p;
      ...
      <div className="card">
        ...
        <div className="form-group">
          ...
          <select className="form-select" value={paddock} onChange={e=>setPaddock(e.target.value)}>
            ...
            {sortedPaddocks(paddocks).filter(p=>!p.resting).map(p=><option key={p.id} value={p.id}>{p.name}</option>)}
          </select>
        </div>
        {isNewLambs ? (
          <div className="form-row">
            ...
            <div className="form-group"><label className="form-label">🐄 {T.cattleCalves}</label><input className="form-input" type="number" min="0" value={cattleCalves} onChange={e=>setCattleCalves(e.target.value)} placeholder="0"/></div>
          </div>
        ) : (
          <>
            ...
            <GoatCategoryGrid label={`🐐 ${T.goats}`} values={moveGo} onChange={(c,v)=>setMoveGo(prev=>({...prev,[c]:Number(v)||0}))} max={type==="sold"?fromP?.goats:undefined}/>
        ...
        <div className="form-group"><label className="form-label">{T.notes}</label><textarea className="form-textarea" rows={2} value={notes} onChange={e=>setNotes(e.target.value)} placeholder={T.notesPlaceholder}/></div>
    ...
    <>
        ...
        <div className="form-row">
          ...
          <div className="form-group">
            ...
            <select className="form-select" value={to} onChange={e=>setTo(e.target.value)}>
              <option value="">{T.select}</option>
              {sortedPaddocks(paddocks).filter(p=>p.id!==from && !p.resting).map(p=><option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
          </div>
        </div>
        <CategoryGrid label={`🐑 ${T.sheep}`} values={moveSh} onChange={(c,v)=>setMoveSh(prev=>({...prev,[c]:Number(v)||0}))} max={fromP?.sheep}/>
        <GoatCategoryGrid label={`🐐 ${T.goats}`} values={moveGo} onChange={(c,v)=>setMoveGo(prev=>({...prev,[c]:Number(v)||0}))} max={fromP?.goats}/>
      ...
      {fromP && (
        <div className="card" style={{marginTop:10,background:"var(--bg)"}}>
          ...
          <div className="stats">
            {CATS.map(c=>(<div key={c} className="stat"><div style={{fontSize:".7rem",color:"var(--muted)"}}>{CATLABEL()[c]}/{GOAT_CATLABEL()[c]}</div><div className="stat-val sheep-c">{fromP.sheep[c]}</div><div className="stat-val goats-c" style={{fontSize:".9rem"}}>{fromP.goats[c]}</div></div>))}
    ...
    <>
      ...
      <div className="card" style={{marginBottom:14, ...(totalMissing>0 ? {borderColor:"var(--amber)", background:"#fff8f0"} : {})}}>
        ...
        {showFound && totalMissing>0 && (
          <div className="extra-section" style={{marginTop:10}}>
            ...
            {totalCat(missing.goats)>0 && <GoatCategoryGrid label={`🐐 ${T.goats}`} values={foundGo} onChange={(c,v)=>setFoundGo(prev=>({...prev,[c]:Number(v)||0}))} max={missing.goats}/>}
            ...
            <div className="form-group"><label className="form-label">{T.notes}</label><textarea className="form-textarea" rows={2} value={foundNotes} onChange={e=>setFoundNotes(e.target.value)} placeholder={T.notesPlaceholder}/></div>
      ...
      <div className="card">
        ...
        <div className="form-group">
          ...
          <select className="form-select" value={paddock} onChange={e=>setPaddock(e.target.value)}>
            <option value="">{T.select}</option>
            {sortedPaddocks(paddocks).map(p=><option key={p.id} value={p.id}>{p.name}</option>)}
          </select>
        </div>
        <CategoryGrid label={`🐑 ${T.sheep}`} values={sheep} onChange={(c,v)=>setSheep(prev=>({...prev,[c]:Number(v)||0}))}/>
        <GoatCategoryGrid label={`🐐 ${T.goats}`} values={goats} onChange={(c,v)=>setGoats(prev=>({...prev,[c]:Number(v)||0}))}/>
  ...
  return (
    <>
      ...
      <div className="card">
        ...
        <div className="form-row">
          ...
          <div className="form-group">
            ...
            <select className="form-select" value={cat} onChange={e=>setCat(e.target.value)}>
              {CATS.map(c=><option key={c} value={c}>{(species==="goats"?GOAT_CATLABEL():CATLABEL())[c]}</option>)}
        ...
        <div className="form-row">
          <div className="form-group"><label className="form-label">{T.treatmentName}</label><input className="form-input" value={tname} onChange={e=>setTname(e.target.value)} placeholder={getLang()==="en"?"e.g. Ivermectin":"bv. Ivermectien"}/></div>
          <div className="form-group"><label className="form-label">{T.dosage}</label><input className="form-input" value={dosage} onChange={e=>setDosage(e.target.value)} placeholder={getLang()==="en"?"e.g. 1ml/10kg":"bv. 1ml/10kg"}/></div>
        </div>
        <div className="form-row">
          <div className="form-group">
          ...
          <div className="form-group"><label className="form-label">{T.administeredBy}</label><input className="form-input" value={adminBy} onChange={e=>setAdminBy(e.target.value)} placeholder={T.adminByPlaceholder}/></div>
        </div>
        <div className="form-group"><label className="form-label">{T.notes}</label><textarea className="form-textarea" rows={2} value={notes} onChange={e=>setNotes(e.target.value)} placeholder={T.notesPlaceholder}/></div>
        <label className="linked-check">
        ...
        {linkedToMove && (
          <div className="form-group">
            ...
            <select className="form-select" value={destPaddock} onChange={e=>setDestPad(e.target.value)}>
              <option value="">{T.select}</option>
              {sortedPaddocks(paddocks).filter(p=>p.id!==paddock).map(p=><option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
          </div>
        )}
        <AdditionalRecords extra={extra} setExtra={setExtra} />
...
// ── Take ──────────────────────────────────────────────────────────────────────
function Take({ tasks, setTasks, paddocks, role, addEvent, pushUndo, snapNow }) {
  const [,] = useLang();
  ...
  const add = () => {
    ...
    pushUndo(getLang()==="en"?"Task assigned":"Taak toegewys", snapNow());
  ...
  const complete = (id) => {
    pushUndo(getLang()==="en"?"Task completed":"Taak voltooi", snapNow());
            ...
            <div className="card" key={t.id} style={{borderColor:od?"var(--rust)":"var(--border)"}}>
              <div className="card-header">
                <div style={{flex:1}}>
                  ...
                  {t.dueDate&&<div style={{fontSize:".8rem",marginTop:2,color:od?"var(--rust)":"var(--muted)",fontWeight:od?700:400}}>{od?`⚠️ ${T.overduePrefix} `:`${T.dueDatePrefix} `}{fmtDate(t.dueDate)}</div>}
                </div>
              ...
              <div style={{fontSize:".73rem",color:"var(--muted)",marginTop:6,marginBottom:t.status==="pending"?8:0}}>{fmtDate(t.createdAt)}{t.doneAt?` · ${T.doneAt} ${fmtTime(t.doneAt)}`:""}</div>
              {t.status==="pending"&&(
                <div style={{display:"flex",gap:8}}>
                  <button className="btn btn-done btn-sm" onClick={()=>complete(t.id)}>{T.markDone}</button>
                  {role==="admin"&&<button className="btn btn-ghost btn-sm" style={{color:"var(--rust)"}} onClick={()=>{if(confirm(T.confirmDeleteTask)){pushUndo(getLang()==="en"?"Task deleted":"Taak verwyder",snapNow());setTasks(prev=>prev.filter(x=>x.id!==t.id));}}}>❌</button>}
                </div>
              )}
            </div>
          );
      ...
      {modal&&(
        <div className="overlay" onClick={()=>setModal(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="form-group"><label className="form-label">{T.taskDesc}</label><input className="form-input" value={title} onChange={e=>setTitle(e.target.value)} placeholder={T.taskTitlePlaceholder} autoFocus/></div>
            <div className="form-row">
              <div className="form-group"><label className="form-label">{T.assignTo}</label><input className="form-input" value={assign} onChange={e=>setAssign(e.target.value)} placeholder={T.workerNamePlaceholder}/></div>
            ...
            <div className="form-group"><label className="form-label">{T.paddock}</label><select className="form-select" value={paddock} onChange={e=>setPaddock(e.target.value)}><option value="">{T.noSpecific}</option>{sortedPaddocks(paddocks).map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
            <div className="form-group"><label className="form-label">{T.notes}</label><textarea className="form-textarea" rows={2} value={notes} onChange={e=>setNotes(e.target.value)} placeholder={T.taskNotesPlaceholder}/></div>
...
// ── Probleme ──────────────────────────────────────────────────────────────────
function Probleme({ issues, setIssues, paddocks, addEvent, pushUndo, snapNow }) {
  const [,] = useLang();
  const [modal, setModal] = useState(false);
  ...
  const add = () => {
    if(!title.trim()) return;
    pushUndo(getLang()==="en"?"Issue reported":"Probleem gerapporteer", snapNow());
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {shown.map(iss=>(
          <div className="card" key={iss.id} style={{borderColor:iss.status==="open"?"var(--amber)":"var(--border)"}}>
            ...
            <div style={{fontSize:".73rem",color:"var(--muted)",marginTop:6,marginBottom:iss.status==="open"?8:0}}>{fmtTime(iss.createdAt)}{iss.resolvedAt?` · ${T.resolvedAt} ${fmtTime(iss.resolvedAt)}`:""}</div>
            {iss.status==="open"&&(
              <div style={{display:"flex",gap:8}}>
                <button className="btn btn-done btn-sm" onClick={()=>{pushUndo(getLang()==="en"?"Issue resolved":"Probleem opgelos",snapNow());setIssues(prev=>prev.map(i=>i.id===iss.id?{...i,status:"resolved",resolvedAt:nowISO()}:i));addEvent({type:"issue",action:"resolved",issueTitle:iss.title});}}>{T.markResolved}</button>
                <button className="btn btn-ghost btn-sm" style={{color:"var(--rust)"}} onClick={()=>{if(confirm(T.confirmDeleteIssue)){pushUndo(getLang()==="en"?"Issue deleted":"Probleem verwyder",snapNow());setIssues(prev=>prev.filter(i=>i.id!==iss.id));}}}> ❌</button>
              </div>
            )}
          </div>
        ))}
      ...
      {modal&&(
        <div className="overlay" onClick={()=>setModal(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="form-group"><label className="form-label">{getLang()==="en"?"Description":"Beskrywing"}</label><input className="form-input" value={title} onChange={e=>setTitle(e.target.value)} placeholder={T.issueDescPlaceholder} autoFocus/></div>
            <div className="form-row">
            ...
            <div className="form-group"><label className="form-label">{T.notes}</label><textarea className="form-textarea" rows={2} value={notes} onChange={e=>setNotes(e.target.value)} placeholder={T.issueNotesPlaceholder}/></div>
            <div className="form-group"><label className="form-label">{getLang()==="en"?"Photo":"Foto"}</label>
              <div className="photo-upload" onClick={()=>fileRef.current?.click()}>{photo?<img src={photo} alt="v" style={{maxHeight:90,borderRadius:6}}/>:T.addPhoto}</div>
...
// ── Geskiedenis ───────────────────────────────────────────────────────────────
function Geskiedenis({ events, setEvents, paddocks, role, pushUndo, snapNow }) {
  const [,] = useLang();
  ...
  const deleteEvent = (id) => {
    if (!confirm(getLang()==="en" ? "Delete this history record?" : "Hierdie rekord verwyder?")) return;
    pushUndo(getLang()==="en" ? "Event deleted" : "Gebeurtenis verwyder", snapNow());
            ...
            <div key={ev.id} style={{position:"relative"}}>
              ...
              {role==="admin" && (
                <button
                  ...
                  style={{position:"absolute",top:10,right:4,background:"none",border:"none",
                    color:"var(--border)",fontSize:"1.1rem",cursor:"pointer",padding:"2px 6px",
                    borderRadius:6,lineHeight:1,transition:"color .15s"}}
                  title={getLang()==="en"?"Delete record":"Verwyder rekord"}
...
function Analitiek({ paddocks, events, tasks, issues, treatments, settings }) {
  ...
  const totalNewLambs = newLambEvts.reduce((s,e)=>s+(e.sheepLambs||0)+(e.goatLambs||0)+(e.cattleCalves||0),0);
  ...
  const monthlyData = (() => {
    ...
    [...events].sort((a,b)=>new Date(a.ts)-new Date(b.ts)).forEach(ev => {
      ...
      if(ev.type==="newLambs") months[m].born    += (ev.sheepLambs||0)+(ev.goatLambs||0)+(ev.cattleCalves||0);
    });
  ...
  const lsuPaddockData = paddocks.map(p=>{
    ...
    return {name:p.name,load:lsu.loadPct!==null?Math.round(lsu.loadPct):null,
            ...
            budget:lsu.budget!==null?+lsu.budget.toFixed(0):null,
            consumed:lsu.consumed!==null?+lsu.consumed.toFixed(0):null,
    ...
    <>
      ...
      <div className="card" style={{marginBottom:12}}>
        ...
        <table className="data-table"><tbody>
          ...
          <tr><td>{T.animalsBought}</td><td>{bought.reduce((s,e)=>s+totalCat(e.sheep??{})+totalCat(e.goats??{})+totalCattle(e.cattle||emptyCattle()),0)}</td></tr>
          <tr><td>{T.animalsSold}</td><td>{sold.reduce((s,e)=>s+totalCat(e.sheep??{})+totalCat(e.goats??{})+totalCattle(e.cattle||emptyCattle()),0)}</td></tr>
          <tr><td>{T.newLambsRec}</td><td>{newL.reduce((s,e)=>s+(e.sheepLambs||0)+(e.goatLambs||0)+(e.cattleCalves||0),0)}</td></tr>
          <tr><td>{T.deathsRec}</td><td>{deaths.reduce((s,e)=>s+totalCat(e.sheep??{})+totalCat(e.goats??{})+totalCattle(e.cattle||emptyCattle()),0)}</td></tr>
          <tr><td>{T.lostRec}</td><td>{lost.reduce((s,e)=>s+totalCat(e.sheep??{})+totalCat(e.goats??{})+totalCattle(e.cattle||emptyCattle()),0)}</td></tr>
          <tr><td>{T.treatmentsLbl}</td><td>{treatments.length}</td></tr>
          ...
          <table className="data-table">
            ...
            <tbody>{paddocks.map(p=>{
              ...
              return <tr key={p.id}>
                ...
                <td>{lsu.budget!==null?lsu.budget.toFixed(0):"-"}</td>
                <td>{lsu.consumed!==null?lsu.consumed.toFixed(1):"-"}</td>
      ...
      <div className="card">
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          ...
          <button className="btn btn-amber" onClick={()=>dl([
            ...
            ...events.map(ev=>{
              let det="",loc="";
              if(ev.type==="movement"){det=`${T.moved}: ${totalCat(ev.sheep??{})} ${T.sheep2}, ${totalCat(ev.goats??{})} ${T.goats2}${totalCattle(ev.cattle||emptyCattle())>0?`, ${totalCattle(ev.cattle||emptyCattle())} ${T.cattle2}`:""}`;loc=`${ev.fromName||""}→${ev.toName||""}`;}
              else if(ev.type==="bought"){det=`${T.bought2}: ${totalCat(ev.sheep??{})} ${T.sheep2}, ${totalCat(ev.goats??{})} ${T.goats2}${totalCattle(ev.cattle||emptyCattle())>0?`, ${totalCattle(ev.cattle||emptyCattle())} ${T.cattle2}`:""}`;loc=ev.paddockName||"";}
              else if(ev.type==="sold"){det=`${T.sold2}: ${totalCat(ev.sheep??{})} ${T.sheep2}, ${totalCat(ev.goats??{})} ${T.goats2}${totalCattle(ev.cattle||emptyCattle())>0?`, ${totalCattle(ev.cattle||emptyCattle())} ${T.cattle2}`:""}`;loc=ev.paddockName||"";}
              else if(ev.type==="newLambs"){det=`${T.newLambs}: ${ev.sheepLambs||0}+${ev.goatLambs||0}+${ev.cattleCalves||0}`;loc=ev.paddockName||"";}
              else if(ev.type==="death"){det=`${T.dead}: ${totalCat(ev.sheep??{})+totalCat(ev.goats??{})+totalCattle(ev.cattle||emptyCattle())}${ev.cause?` (${ev.cause})`:""}`;loc=ev.paddockName||"";}
              else if(ev.type==="lost"){det=`${T.missing}: ${totalCat(ev.sheep??{})+totalCat(ev.goats??{})+totalCattle(ev.cattle||emptyCattle())}`;loc=ev.paddockName||"";}
              else if(ev.type==="found"){det=`${T.found}: ${totalCat(ev.sheep??{})+totalCat(ev.goats??{})+totalCattle(ev.cattle||emptyCattle())}`;loc=ev.paddockName||"";}
...
function EventRow({ ev }) {
  ...
  const configs = {
    ...
    newLambs:  {icon:"🐣",cls:"ev-treat",title:`${T.newLambs}: ${(ev.sheepLambs||0)+(ev.goatLambs||0)+(ev.cattleCalves||0)} ${T.lambs2}`, meta:ev.paddockName||""},
