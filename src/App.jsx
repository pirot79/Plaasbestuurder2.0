...
function calcLSU(paddock, settings) {
  ...
  const totalLSU  = sheepLSU + goatLSU + cattleLSU;

  const capacity  = (paddock.hectares && settings.haPerLSU)
    ? paddock.hectares / settings.haPerLSU : null;
  const loadPct   = capacity ? (totalLSU / capacity) * 100 : null;

  return { sheepLSU, goatLSU, cattleLSU, totalLSU, capacity, loadPct };
}
...
function LSUBar({ paddock, settings }) {
  const { totalLSU, capacity, loadPct } = calcLSU(paddock, settings);
  ...
  return (
    <div className="lsu-bar-wrap">
      <div className="lsu-bar-label">
        <span style={{fontWeight:600,fontSize:".73rem"}}>⚖️ {totalLSU.toFixed(1)} / {capacity.toFixed(1)} LSU</span>
        <span style={{color,fontWeight:700,fontSize:".73rem"}}>{loadPct.toFixed(0)}% — {label}</span>
      </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [p,e,t,i,tr,s] = await Promise.all([
        ...
        load(KEYS.tr, [], farmId),
        load(KEYS.s, { farmName:"", haPerLSU:"", sheepFrame:"medium", goatFrame:"medium", cattleFrame:"medium" }, farmId),
      ]);
  ...
  const addInspection = (paddockId, sheepScore, goatScore, cattleScore, paddockCond, observations) => {
    const ts = nowISO();
    ...
    setPaddocks(prev => prev.map(p => p.id===paddockId
      ? { ...p, paddockCondition:paddockCond!==null?paddockCond:p.paddockCondition,
               conditionHistory:[entry,...(p.conditionHistory||[])] } : p));
    addEvent({ type:"inspection", paddock:paddockId,
      paddockName:paddocks.find(p=>p.id===paddockId)?.name,
      sheepScore, goatScore, paddockCond, observations });
  };

  const applyExtra = (extra, sourcePaddockId, destPaddockId = null) => {
    ...
    if (lostSh+lostGo+lostCa > 0) {
      setPaddocks(prev => prev.map(p => p.id===sourcePaddockId
        ? {...p, sheep:subCat(p.sheep,extra.lost.sheep), goats:subCat(p.goats,extra.lost.goats),
               cattle:subCattle(p.cattle||emptyCattle(),extra.lost.cattle||emptyCattle())} : p));
      addEvent({type:"lost", paddock:sourcePaddockId, paddockName:paddocks.find(p=>p.id===sourcePaddockId)?.name, sheep:extra.lost.sheep, goats:extra.lost.goats, cattle:extra.lost.cattle});
    }
    ...
    if (deadSh+deadGo+deadCa > 0) {
      setPaddocks(prev => prev.map(p => p.id===sourcePaddockId
        ? {...p, sheep:subCat(p.sheep,extra.dead.sheep), goats:subCat(p.goats,extra.dead.goats),
               cattle:subCattle(p.cattle||emptyCattle(),extra.dead.cattle||emptyCattle())} : p));
      addEvent({type:"death", paddock:sourcePaddockId, paddockName:paddocks.find(p=>p.id===sourcePaddockId)?.name, sheep:extra.dead.sheep, goats:extra.dead.goats, cattle:extra.dead.cattle, cause:extra.deadCause});
    }
    ...
    if (isSame) {
      if (hasScores || hasPaddock || hasObs) {
        const entry = { id:uid(), ts, source:evtSrc, sheepScore:extra.sheepScore, goatScore:extra.goatScore, cattleScore:extra.cattleScore, paddockCond:extra.paddockCond, observations:extra.observations };
        setPaddocks(prev => prev.map(p => p.id===sourcePaddockId
          ? { ...p, paddockCondition:hasPaddock?extra.paddockCond:p.paddockCondition, conditionHistory:[entry,...(p.conditionHistory||[])] } : p));
      }
    } else {
      if (hasScores) {
        const destEntry = { id:uid(), ts, source:evtSrc, sheepScore:extra.sheepScore, goatScore:extra.goatScore, cattleScore:extra.cattleScore, paddockCond:null, observations:extra.observations };
        setPaddocks(prev => prev.map(p => p.id===destPaddockId ? { ...p, conditionHistory:[destEntry,...(p.conditionHistory||[])] } : p));
      }
      if (hasPaddock || hasObs) {
        const srcEntry = { id:uid(), ts, source:evtSrc, sheepScore:null, goatScore:null, paddockCond:extra.paddockCond, observations:extra.observations };
        setPaddocks(prev => prev.map(p => p.id===sourcePaddockId
  ...
  const handleSignOut = async () => {
    ...
    setIssues([]); setTreatments([]); setSettings({ farmName:"", haPerLSU:"", sheepFrame:"medium", goatFrame:"medium", cattleFrame:"medium" });
  };
  ...
  // Farm-wide LSU totals for dashboard chip
  const farmTotalLSU     = paddocks.reduce((s,p)=>s+calcLSU(p,settings).totalLSU,0);
  const farmTotalCapacity= paddocks.reduce((s,p)=>{ const c=calcLSU(p,settings).capacity; return s+(c||0); },0);
  const farmLoadPct      = farmTotalCapacity>0 ? (farmTotalLSU/farmTotalCapacity)*100 : null;
  ...
  const ctx = { paddocks, setPaddocks, events, setEvents, tasks, setTasks,
                issues, setIssues, treatments, setTreatments, role, addEvent, applyExtra, addInspection, settings };
  ...
  const submit = async () => {
    ...
    try {
      if (mode === "signup") {
        ...
        await save(KEYS.s, { farmName: farmName.trim(), haPerLSU:"", sheepFrame:"medium", goatFrame:"medium", cattleFrame:"medium" }, uid2);
      } else {
...
// ── Paddock Card ──────────────────────────────────────────────────────────────
function PaddockCard({ p, role, settings, onDelete, onReset, onClick }) {
  const [,] = useLang();
...
// ── Paddocks page ─────────────────────────────────────────────────────────────
function Paddocks({ paddocks, setPaddocks, role, addEvent, addInspection, settings }) {
  const [,] = useLang();
  ...
  const addPaddock = () => {
    ...
    setName(""); setHectares(""); setAddModal(false);
  };
...
function Transaksies({ paddocks, setPaddocks, addEvent }) {
  ...
  const reset = () => { setPaddock(""); setMoveSh(emptyCat()); setMoveGo(emptyCat()); setMoveCa(emptyCattle()); setSheepLambs(""); setGoatLambs(""); setNotes(""); };
  ...
  const submit = () => {
    ...
    if(isNewLambs) {
      ...
      setPaddocks(prev=>prev.map(p=>{
        ...
        return {...p, sheep:{...p.sheep,lambs:(p.sheep.lambs||0)+sl}, goats:{...p.goats,lambs:(p.goats.lambs||0)+gl}, arrivedAt: wasEmpty&&(sl+gl)>0 ? ts : p.arrivedAt};
      }));
      ...
      setPaddocks(prev=>prev.map(p=>{
        if(p.id!==paddock) return p;
        const wasEmpty = totalCat(p.sheep)+totalCat(p.goats)===0;
        return {...p, sheep:addCat(p.sheep,moveSh), goats:addCat(p.goats,moveGo), cattle:addCattle(p.cattle||emptyCattle(),moveCa), arrivedAt: wasEmpty ? ts : p.arrivedAt};
      }));
      addEvent({type:"bought", paddock, paddockName, sheep:moveSh, goats:moveGo, cattle:moveCa, notes});
    } else {
      setPaddocks(prev=>prev.map(p=>{
        if(p.id!==paddock) return p;
        const newSheep=subCat(p.sheep,moveSh), newGoats=subCat(p.goats,moveGo), newCattle=subCattle(p.cattle||emptyCattle(),moveCa);
        const isEmpty=totalCat(newSheep)+totalCat(newGoats)+totalCattle(newCattle)===0;
        return {...p, sheep:newSheep, goats:newGoats, cattle:newCattle, arrivedAt: isEmpty?null:p.arrivedAt};
      }));
      addEvent({type:"sold", paddock, paddockName, sheep:moveSh, goats:moveGo, cattle:moveCa, notes});
    }
...
function Bewegings({ paddocks, setPaddocks, addEvent, applyExtra }) {
  ...
  const submit = () => {
    ...
    setPaddocks(prev=>prev.map(p=>{
      if(p.id===from) return {...p,sheep:subCat(p.sheep,moveSh),goats:subCat(p.goats,moveGo),cattle:subCattle(p.cattle||emptyCattle(),moveCa)};
      if(p.id===to)   return {...p,sheep:addCat(p.sheep,moveSh),goats:addCat(p.goats,moveGo),cattle:addCattle(p.cattle||emptyCattle(),moveCa),arrivedAt:ts};
      return p;
    }));
    addEvent({type:"movement",from,to,fromName,toName,sheep:moveSh,goats:moveGo,cattle:moveCa,notes,
      sheepScore:extra.sheepScore,goatScore:extra.goatScore,paddockCond:extra.paddockCond,observations:extra.observations});
    applyExtra(extra, from, to);
    setFrom("");setTo("");setMoveSh(emptyCat());setMoveGo(emptyCat());setMoveCa(emptyCattle());setNotes("");setExtra(emptyExtra());
...
function Verliese({ paddocks, setPaddocks, addEvent }) {
  ...
  const submit = () => {
    ...
    setPaddocks(prev=>prev.map(p=>p.id===paddock?{...p,sheep:subCat(p.sheep,sheep),goats:subCat(p.goats,goats),cattle:subCattle(p.cattle||emptyCattle(),cattle)}:p));
    addEvent({type, paddock, paddockName, sheep, goats, cattle, cause, notes});
    setPaddock("");setSheep(emptyCat());setGoats(emptyCat());setCattle(emptyCattle());setCause("");setNotes("");
    setSaved(true); setTimeout(()=>setSaved(false),2500);
  };
...
function Behandelings({ paddocks, setPaddocks, treatments, setTreatments, addEvent, applyExtra }) {
  ...
  const submit = () => {
    ...
    addEvent({type:"treatment",paddock,paddockName,species,category:cat,count:Number(count),treatment:tname,method,administeredBy:adminBy,
      sheepScore:extra.sheepScore,goatScore:extra.goatScore,cattleScore:extra.cattleScore,linkedToMove,destName});
    applyExtra(extra, paddock, linkedToMove&&destPaddock?destPaddock:null);
    setP("");setSpecies("sheep");setCat("ewes");setCount("");setTname("");setDosage("");setMethod("");setAdminBy("");setNotes("");setExtra(emptyExtra());setLinked(false);setDestPad("");
    setSaved(true); setTimeout(()=>setSaved(false),2500);
  };
...
const HISTORY_FILTERS = () => [
  ...
  {id:"lost",label:T.lost},{id:"treatment",label:T.treatments},{id:"task",label:T.tasks},
...
function Analitiek({ paddocks, events, tasks, issues, treatments, settings }) {
  ...
  const CHART_COLORS = {
    sheep:"#4a5c3a", goats:"#c17f2a", cattle:"#2a7a6a",
    green:"#2a6a2a", amber:"#c17f2a", rust:"#a83c2a", sky:"#2a6c8a",
    p1:"#4a5c3a", p2:"#c17f2a", p3:"#2a7a6a", p4:"#2a6c8a", p5:"#a83c2a",
  };
  ...
  const boughtEvts  = events.filter(e=>e.type==="bought");
  const soldEvts    = events.filter(e=>e.type==="sold");
  ...
  const totalBought = boughtEvts.reduce((s,e)=>s+totalCat(e.sheep||{})+totalCat(e.goats||{})+totalCattle(e.cattle||emptyCattle()),0);
  const totalSold   = soldEvts.reduce((s,e)=>s+totalCat(e.sheep||{})+totalCat(e.goats||{})+totalCattle(e.cattle||emptyCattle()),0);
  ...
  const lossPct     = totalHerd>0 ? ((totalLost/(totalHerd+totalLost))*100).toFixed(1) : null;

  // Monthly herd trend from events
  const monthlyData = (() => {
    const months = {};
    const sorted = [...events].sort((a,b)=>new Date(a.ts)-new Date(b.ts));
    sorted.forEach(ev => {
      const m = ev.ts.slice(0,7);
      if(!months[m]) months[m]={month:m,bought:0,sold:0,deaths:0,lost:0,born:0};
      ...
      if(ev.type==="lost")   months[m].lost   += totalCat(ev.sheep||{})+totalCat(ev.goats||{})+totalCattle(ev.cattle||emptyCattle());
      if(ev.type==="newLambs") months[m].born += (ev.sheepLambs||0)+(ev.goatLambs||0);
    });
    return Object.values(months).slice(-6).map(m=>({...m, month:m.month.slice(5)+"/"+m.month.slice(2,4), net:m.bought+m.born-m.sold-m.deaths-m.lost}));
  })();

  const speciesBarData = [
    { name:`🐑 ${T.sheep}`,  value:totSheep,  fill:CHART_COLORS.sheep },
    { name:`🐐 ${T.goats}`,  value:totGoats,  fill:CHART_COLORS.goats },
    { name:`🐄 ${T.cattle}`, value:totCattle, fill:CHART_COLORS.cattle },
  ].filter(d=>d.value>0);

  // ── Health & Condition derivations ─────────────────────────────────────────
  const condTrendData = paddocks.filter(p=>(p.conditionHistory||[]).length>0).map(p=>{
    const hist = (p.conditionHistory||[]).slice(0,8).reverse();
    return { name:p.name, data:hist.map((h,i)=>({ i, sheep:h.sheepScore, goat:h.goatScore })) };
  });
  ...
  const deathCauseData = Object.entries(deathCauses).map(([name,value])=>({name,value}));
  const PIE_COLORS = [CHART_COLORS.rust,CHART_COLORS.amber,CHART_COLORS.sky,CHART_COLORS.sheep,CHART_COLORS.teal,"#8884d8"];

  // ── Paddock Management derivations ─────────────────────────────────────────
  const lsuPaddockData = paddocks.map(p=>{
    const lsu = calcLSU(p, settings);
    return { name:p.name, load:lsu.loadPct!==null?Math.round(lsu.loadPct):null, lsu:+lsu.totalLSU.toFixed(1), cap:lsu.capacity?+lsu.capacity.toFixed(1):null };
  }).filter(d=>d.load!==null).sort((a,b)=>b.load-a.load);

  const condPaddockData = paddocks.map(p=>({
    name:p.name,
    cond:p.paddockCondition??null,
    days:daysSince(p.arrivedAt)??0,
  })).filter(d=>d.cond!==null).sort((a,b)=>a.cond-b.cond);
  ...
  const avgTaskDays = doneTasks.filter(t=>t.doneAt&&t.createdAt).length>0
    ? (doneTasks.filter(t=>t.doneAt&&t.createdAt).reduce((s,t)=>s+((new Date(t.doneAt)-new Date(t.createdAt))/86400000),0)/doneTasks.filter(t=>t.doneAt&&t.createdAt).length).toFixed(1)
    : null;
  ...
  const avgIssueDays = resolvedIssues.length>0
    ? (resolvedIssues.reduce((s,i)=>s+((new Date(i.resolvedAt)-new Date(i.createdAt))/86400000),0)/resolvedIssues.length).toFixed(1)
    : null;
  ...
  const taskStatusData = [
    { name:T.done,    value:doneTasks.length,                                          fill:CHART_COLORS.green },
    { name:T.overdue, value:pendTasks.filter(t=>isOverdue(t.dueDate)).length,          fill:CHART_COLORS.rust  },
    { name:T.pending, value:pendTasks.filter(t=>!isOverdue(t.dueDate)).length,         fill:CHART_COLORS.amber },
  ].filter(d=>d.value>0);
  ...
  const noData = (arr) => arr.length===0 ? <div style={{color:"var(--muted)",fontSize:".82rem",padding:"12px 0"}}>{T.noData}</div> : null;

  const CustomTooltip = ({active,payload,label}) => {
    if(!active||!payload?.length) return null;
    return (
      <div style={{background:"var(--surface)",border:"1px solid var(--border)",borderRadius:8,padding:"8px 12px",fontSize:".78rem"}}>
  ...
  return (
    <>
            ...
            <div className="stat-panel-card">
              ...
              <div className="chart-wrap" style={{height:160}}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={speciesBarData} margin={{top:4,right:8,left:-20,bottom:0}}>
                    <XAxis dataKey="name" tick={{fontSize:11}} />
                    <YAxis tick={{fontSize:10}} />
                    <Tooltip content={<CustomTooltip/>}/>
                    <Bar dataKey="value" radius={[6,6,0,0]}>
                      {speciesBarData.map((d,i)=><Cell key={i} fill={d.fill}/>)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
              ...
              <div className="chart-wrap" style={{height:180}}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={monthlyData} margin={{top:4,right:8,left:-20,bottom:0}}>
                    ...
                    <Bar dataKey="bought" name={T.bought} fill={CHART_COLORS.green} stackId="a" radius={[0,0,0,0]}/>
                    <Bar dataKey="born"   name={T.newLambs} fill={CHART_COLORS.sky} stackId="a"/>
                    <Bar dataKey="sold"   name={T.sold}   fill={CHART_COLORS.amber} stackId="b"/>
                    <Bar dataKey="deaths" name={T.death}  fill={CHART_COLORS.rust}  stackId="b"/>
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
      ...
      {panel==="health" && (
        <>
          {condTrendData.length > 0 ? (
            <div className="stat-panel-card">
              <div className="stat-panel-title">{T.condTrend} — {T.sheepScore}</div>
              <div className="chart-wrap" style={{height:200}}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart margin={{top:4,right:8,left:-20,bottom:0}}>
                    <XAxis dataKey="i" type="number" tick={false}/>
                    <YAxis domain={[1,5]} ticks={[1,2,3,4,5]} tick={{fontSize:10}}/>
                    <Tooltip content={<CustomTooltip/>}/>
                    <Legend wrapperStyle={{fontSize:".72rem"}}/>
                    {condTrendData.slice(0,4).map((p,i)=>(
                      <Line key={p.name} data={p.data} type="monotone" dataKey="sheep"
                        name={p.name} stroke={Object.values(CHART_COLORS)[i]}
          ...
          {topTreatmentsData.length > 0 && (
            <div className="stat-panel-card">
              ...
              <div className="chart-wrap" style={{height:160}}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={topTreatmentsData} layout="vertical" margin={{top:4,right:8,left:8,bottom:0}}>
                    <XAxis type="number" tick={{fontSize:10}}/>
                    <YAxis type="category" dataKey="name" tick={{fontSize:10}} width={90}/>
                    <Tooltip content={<CustomTooltip/>}/>
                    <Bar dataKey="count" fill={CHART_COLORS.teal} radius={[0,6,6,0]}/>
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          {deathCauseData.length > 0 && (
            <div className="stat-panel-card">
              ...
              <div className="chart-wrap" style={{height:180}}>
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={deathCauseData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={65} label={({name,percent})=>`${name} ${(percent*100).toFixed(0)}%`} labelLine={false}>
                      {deathCauseData.map((_,i)=><Cell key={i} fill={PIE_COLORS[i%PIE_COLORS.length]}/>)}
                    </Pie>
                    <Tooltip content={<CustomTooltip/>}/>
                  </PieChart>
                </ResponsiveContainer>
        ...
        <>
          {lsuPaddockData.length > 0 ? (
            <div className="stat-panel-card">
              ...
              <div className="chart-wrap" style={{height: Math.max(160, lsuPaddockData.length*42)}}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={lsuPaddockData} layout="vertical" margin={{top:4,right:40,left:8,bottom:0}}>
                    <XAxis type="number" domain={[0,100]} tickFormatter={v=>`${v}%`} tick={{fontSize:10}}/>
                    <YAxis type="category" dataKey="name" tick={{fontSize:10}} width={80}/>
                    <Tooltip content={<CustomTooltip/>} formatter={(v)=>`${v}%`}/>
                    <Bar dataKey="load" name={T.lsuLoadPct} radius={[0,6,6,0]}>
                      {lsuPaddockData.map((d,i)=>(
                        <Cell key={i} fill={d.load>100?CHART_COLORS.rust:d.load>80?CHART_COLORS.amber:CHART_COLORS.green}/>
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              ...
              {/* Reference line note */}
              <div style={{fontSize:".7rem",color:"var(--muted)",marginTop:6}}>🟢 &lt;80%  🟡 80–100%  🔴 &gt;100%</div>
            </div>
          ) : (
            <div className="stat-panel-card">
          ...
          {condPaddockData.length > 0 && (
            <div className="stat-panel-card">
              ...
              <div className="chart-wrap" style={{height:Math.max(160,condPaddockData.length*40)}}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={condPaddockData} layout="vertical" margin={{top:4,right:40,left:8,bottom:0}}>
                    <XAxis type="number" domain={[0,100]} tickFormatter={v=>`${v}%`} tick={{fontSize:10}}/>
                    <YAxis type="category" dataKey="name" tick={{fontSize:10}} width={80}/>
                    <Tooltip content={<CustomTooltip/>}/>
                    <Bar dataKey="cond" name={T.paddockConditionPct} radius={[0,6,6,0]}>
                      {condPaddockData.map((d,i)=>(
                        <Cell key={i} fill={d.cond>=70?CHART_COLORS.green:d.cond>=40?CHART_COLORS.amber:CHART_COLORS.rust}/>
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
          ...
          {/* Days in paddock ranking */}
          {paddocks.filter(p=>daysSince(p.arrivedAt)!==null).length > 0 && (
            <div className="stat-panel-card">
              <div className="stat-panel-title">📅 {T.daysInPaddock}</div>
              <div className="chart-wrap" style={{height:Math.max(120,paddocks.filter(p=>daysSince(p.arrivedAt)!==null).length*38)}}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={paddocks.filter(p=>daysSince(p.arrivedAt)!==null).sort((a,b)=>(daysSince(b.arrivedAt)||0)-(daysSince(a.arrivedAt)||0)).map(p=>({name:p.name,days:daysSince(p.arrivedAt)}))} layout="vertical" margin={{top:4,right:40,left:8,bottom:0}}>
                    <XAxis type="number" tick={{fontSize:10}}/>
                    <YAxis type="category" dataKey="name" tick={{fontSize:10}} width={80}/>
                    <Tooltip content={<CustomTooltip/>}/>
                    <Bar dataKey="days" name={T.fieldDays} radius={[0,6,6,0]}>
                      {paddocks.filter(p=>daysSince(p.arrivedAt)!==null).sort((a,b)=>(daysSince(b.arrivedAt)||0)-(daysSince(a.arrivedAt)||0)).map((p,i)=>{
                        const d=daysSince(p.arrivedAt)||0;
                        return <Cell key={i} fill={d>=14?CHART_COLORS.rust:d>=7?CHART_COLORS.amber:CHART_COLORS.green}/>;
                      })}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
        ...
        <>
          ...
          {taskStatusData.length > 0 && (
            <div className="stat-panel-card">
              ...
              <div className="chart-wrap" style={{height:180}}>
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={taskStatusData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={65}
                      label={({name,value})=>`${name}: ${value}`} labelLine={false}>
                      {taskStatusData.map((d,i)=><Cell key={i} fill={d.fill}/>)}
                    </Pie>
                    <Tooltip content={<CustomTooltip/>}/>
                    <Legend wrapperStyle={{fontSize:".72rem"}}/>
                  </PieChart>
          ...
          {issueTypeData.length > 0 && (
            <div className="stat-panel-card">
              ...
              <div className="chart-wrap" style={{height:Math.max(140,issueTypeData.length*40)}}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={issueTypeData} layout="vertical" margin={{top:4,right:40,left:8,bottom:0}}>
                    <XAxis type="number" tick={{fontSize:10}}/>
                    <YAxis type="category" dataKey="name" tick={{fontSize:10}} width={90}/>
                    <Tooltip content={<CustomTooltip/>}/>
                    <Bar dataKey="count" fill={CHART_COLORS.amber} radius={[0,6,6,0]}/>
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
          )}
          {noData(issueTypeData)}
        </>
      )}
    </>
  );
...
function Uitvoer({ paddocks, tasks, issues, events, treatments, settings }) {
  ...
  const farmTotalLSU = paddocks.reduce((s,p)=>s+calcLSU(p,settings).totalLSU,0);
  const farmTotalCap = paddocks.reduce((s,p)=>{ const c=calcLSU(p,settings).capacity; return s+(c||0); },0);
    ...
    <>
      ...
      <div className="card" style={{marginBottom:12}}>
        ...
        <table className="data-table"><tbody>
          ...
          {farmTotalCap>0 && <tr><td>⚖️ {T.farmLSU}</td><td><strong>{farmTotalLSU.toFixed(1)} / {farmTotalCap.toFixed(1)} LSU ({farmTotalCap>0?((farmTotalLSU/farmTotalCap)*100).toFixed(0):0}%)</strong></td></tr>}
      ...
      <div className="card" style={{marginBottom:12}}>
        ...
        <div style={{overflowX:"auto"}}>
          <table className="data-table">
            <thead><tr><th>{T.paddock}</th><th>{T.hectares}</th><th>Tot🐑</th><th>Tot🐐</th><th>Tot🐄</th><th>LSU</th><th>{T.lsuCapacity}</th><th>{T.lsuLoadPct}</th><th>{T.fieldPaddockCond}</th><th>{T.fieldDays}</th></tr></thead>
            <tbody>{paddocks.map(p=>{
              ...
              return <tr key={p.id}>
                ...
                <td>{lsu.totalLSU.toFixed(1)}</td>
                <td>{lsu.capacity?lsu.capacity.toFixed(1):"-"}</td>
                <td style={{color:lsu.loadPct!==null?(lsu.loadPct>100?"var(--rust)":lsu.loadPct>80?"var(--amber)":"var(--green)"):"inherit",fontWeight:700}}>{lsu.loadPct!==null?`${lsu.loadPct.toFixed(0)}%`:"-"}</td>
                ...
                <td style={{color:(daysSince(p.arrivedAt)||0)>=14?"var(--rust)":(daysSince(p.arrivedAt)||0)>=7?"var(--amber)":"inherit"}}>{daysSince(p.arrivedAt)??"-"}</td>
              </tr>;
            })}</tbody>
          </table>
        </div>
