...
const srcLabel = (s) => s==="inspection"?"Inspeksie":s==="movement"?"Beweging":"Behandeling";
...
const emptyExtra = () => ({
  ...
  deadCause:"", sheepScore:null, goatScore:null,
...
function PaddockDetailModal({ p, onClose, addInspection }) {
  ...
  const submitInsp = () => {
    ...
    addInspection(p.id, iSheep, iGoat, condVal, iObs);
    setISheep(null); setIGoat(null); setICond(""); setIObs("");
  ...
  return (
    <div className="overlay" onClick={onClose}>
      <div className="modal" onClick={e=>e.stopPropagation()}>
        ...
        {!hasSheep && !hasGoats && (
          <div style={{color:"var(--muted)",fontSize:".85rem",marginBottom:10}}>Geen diere tans</div>
        )}
          ...
          {history.map((h,i) => (
            <div key={h.id||i} className="hist-entry">
              ...
              <div className="hist-scores">
                ...
                {h.sheepScore===null && h.goatScore===null && h.paddockCond===null && (
                  <span style={{fontSize:".78rem",color:"var(--muted)"}}>Geen syfers</span>
                )}
              </div>
        ...
        {showInsp && (
          <div className="extra-section" style={{marginTop:8}}>
            ...
            <button className="btn btn-primary btn-full" onClick={submitInsp}
              disabled={iSheep===null && iGoat===null && iCond===""}>
...
export default function App() {
  ...
  const addInspection = (paddockId, sheepScore, goatScore, paddockCond, observations) => {
    const ts = nowISO();
    const entry = { id:uid(), ts, source:"inspection", sheepScore, goatScore, paddockCond, observations };
  ...
  const applyExtra = (extra, sourcePaddockId, destPaddockId = null) => {
    ...
    const hasScores  = extra.sheepScore!==null || extra.goatScore!==null;
    ...
    if (isSame) {
      if (hasScores || hasPaddock || hasObs) {
        const entry = { id:uid(), ts, source:evtSrc, sheepScore:extra.sheepScore,
          goatScore:extra.goatScore, paddockCond:extra.paddockCond, observations:extra.observations };
        setPaddocks(prev => prev.map(p => p.id===sourcePaddockId
      ...
      if (hasScores) {
        const destEntry = { id:uid(), ts, source:evtSrc, sheepScore:extra.sheepScore,
          goatScore:extra.goatScore, paddockCond:null, observations:extra.observations };
    ...
    <>
      ...
      {paddocks.map(p=>(
        <PaddockCard key={p.id} p={p} role={role}
          ...
          onDelete={role==="admin"
            ? (e)=>{e.stopPropagation();if(confirm(`${p.name} verwyder?`))setPaddocks(prev=>prev.filter(x=>x.id!==p.id));}
            : undefined}
        />
      ))}
...
function Behandelings({ paddocks, setPaddocks, treatments, setTreatments, addEvent, applyExtra }) {
  ...
  const submit = () => {
    ...
    addEvent({type:"treatment",paddock,paddockName,species,category:cat,
      ...
      sheepScore:extra.sheepScore,goatScore:extra.goatScore,linkedToMove,destName});
        ...
        <>
          ...
          {treatments.map(tr=>(
            <div className="card" key={tr.id}>
              ...
              {(tr.sheepScore!==null||tr.goatScore!==null) && (
...
const HISTORY_FILTERS = [
  ...
  {id:"inspection",label:"Inspeksies"},{id:"stockTake",label:"Voorraadopname"},
];
    ...
    <>
      ...
      <div className="card" style={{marginBottom:12}}>
        ...
        <table className="data-table"><tbody>
          ...
          <tr><td>Inspeksies</td><td>{events.filter(e=>e.type==="inspection").length}</td></tr>
      ...
      <div className="card" style={{marginBottom:12}}>
        <div className="card-title" style={{marginBottom:10}}>📍 Kamp Diere &amp; Kondisie</div>
        <div style={{overflowX:"auto"}}>
        <table className="data-table">
          <thead><tr><th>Kamp</th><th>Tot🐑</th><th>Tot🐐</th><th>🐑K</th><th>🐐K</th><th>Kamp%</th><th>Dae</th></tr></thead>
      ...
      <div className="card">
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          ...
          <button className="btn btn-amber" onClick={()=>dl([
            ["Datum","Tipe","Besonderhede","Kamp","Skaap K","Bok K","Kamp%","Waarnemings"],
            ...events.map(ev=>{
              let det="",loc="";
              ...
              else if(ev.type==="treatment"){det=`${ev.treatment||""} — ${ev.count||0} diere${ev.linkedToMove&&ev.destName?` → ${ev.destName}`:""}`;loc=ev.paddockName||"";}
              else if(ev.type==="inspection"){det=`Inspeksie`;loc=ev.paddockName||"";}
              else if(ev.type==="stockTake"){det="Voorraadopname";loc=ev.paddockName||"";}
              else if(ev.type==="task"){det=`Taak ${ev.action==="completed"?"voltooi":"toegewys"}: ${ev.taskTitle||""}`;loc="";}
              else{det=`Probleem ${ev.action==="resolved"?"opgelos":"gerapporteer"}: ${ev.issueTitle||""}`;loc=ev.paddockName||"";}
              return[fmtTime(ev.ts),ev.type,det,loc,ev.sheepScore??'',ev.goatScore??'',ev.paddockCond??'',ev.observations??''];
            })
...
function EventRow({ ev }) {
  const configs = {
    ...
    inspection:{icon:"🔍",cls:"ev-insp", title:`Inspeksie: ${ev.paddockName||""}`,
