...
async function loadClient(farmId) {
  try {
    ...
    // Offline: if we have a cached client, use it but treat status as active
    // so a farmer in the field isn't blocked by a stale suspended/trial-expired state
    if (cached) return { ...cached, status: cached.status === "active" ? "active" : "active" };
    return null;
  }
}
...
export default function App() {
  ...
  // ── Auth with offline bootstrap ──────────────────────────────────────────
  // Firebase auth may never fire when offline. After 4s we fall back to the
  // locally-cached uid so the app loads even without network.
  useEffect(() => {
    const CACHE_KEY = "pb_cached_uid";
    let resolved = false;
    ...
    const offlineTimer = setTimeout(() => {
      if (!resolved) {
        ...
        if (cachedUid) {
          setFarmId(cachedUid);
          const cached = lsGet(cachedUid, "__client__");
          if (cached) setClientDoc(cached);
        }
        setAuthReady(true);
      }
    }, 4000);
    ...
    return onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        localStorage.setItem(CACHE_KEY, user.uid);
        setFarmId(user.uid);
  ...
  const handleSignOut = async () => {
    ...
    localStorage.removeItem("pb_cached_uid");
    await signOut(auth);
...
// ── Transaksies ───────────────────────────────────────────────────────────────
function Transaksies({ paddocks, setPaddocks, addEvent, settings }) {
  const [,] = useLang();
...
// ── Bewegings ─────────────────────────────────────────────────────────────────
function Bewegings({ paddocks, setPaddocks, addEvent, applyExtra, settings }) {
  const [,] = useLang();
  ...
  const submit = () => {
    ...
    setPaddocks(prev=>prev.map(p=>{
      if(p.id===from) {
        // Animals leaving — bank the period for animals that left, restart with remaining
        const newSh = subCat(p.sheep,moveSh);
        const newGo = subCat(p.goats,moveGo);
        const newCa = subCattle(p.cattle||emptyCattle(),moveCa);
        const newLSU = calcCurrentLSU({...p,sheep:newSh,goats:newGo,cattle:newCa}, settings).totalLSU;
      ...
      if(p.id===to) {
        // Animals arriving — bank current period, restart with new total
        const newSh = addCat(p.sheep,moveSh);
        const newGo = addCat(p.goats,moveGo);
        const newCa = addCattle(p.cattle||emptyCattle(),moveCa);
        const newLSU = calcCurrentLSU({...p,sheep:newSh,goats:newGo,cattle:newCa}, settings).totalLSU;
...
// ── Verliese ──────────────────────────────────────────────────────────────────
function Verliese({ paddocks, setPaddocks, addEvent, settings }) {
  const [,] = useLang();
...
// ── Behandelings ──────────────────────────────────────────────────────────────
function Behandelings({ paddocks, setPaddocks, treatments, setTreatments, addEvent, applyExtra, settings }) {
  const [,] = useLang();
